...
V85-V90, V98-V105, V114-V117, V145-V147 and V158-V161 for FTFP, V118-V141 for Bertini and V142-V144 for the
at-rest processes.
...
| the anti-nucleus arm of `PropagateNuclNucl`; the two anti-Xi(1530) decay tables; a 256-track list | what the FTF model still refuses: anti-nucleus projectiles past the string stage, and 0.9% of heavy-ion events at the top of the range until the list is 512 | Phase 3, nearly closed: the model runs to a final state for nucleons, pions, kaons, anti-nucleons and ions on any target with its strong resonances decayed ([`docs/PORTED.md`](docs/PORTED.md) 2.1.11b); nothing calls it from the transport yet |
