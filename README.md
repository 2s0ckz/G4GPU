...
foundations and its whole collision tree, and the whole FTFP chain - the tuned parameters, the Glauber
participants, the diffractive excitation and the annihilation, the Lund string fragmentation, the
string decay and the generator - are on main, validated class by class, and not yet reached by a
particle. FTFP runs to a final state for nucleons, pions, kaons, anti-nucleons and ions; the decay
of its strong resonances, the Binary cascade proper and the Bertini cascade are being written. The
...
driver, test, the viewer and the GUI; runs 83 tests; runs example B1 against a 2M-event Geant4
...
V98-V105 and V114-V117 for FTFP.
...
| `G4CascadeInterface` (Bertini) and the INUCL classes under it | the Bertini cascade | Phase 3, in progress on its branch (P10, P10b); nothing of it is on main yet |
...
tests/        83 tests; test_custom_hook is a whole project, built as a user would build one
