...
particle. FTFP runs to a final state for nucleons, pions, kaons and anti-nucleons; its ion
hand-over, the Binary cascade proper and the Bertini cascade are being written. The
...
entries V37-V45 for Phase 1, V67-V76, V91-V94 and V106-V113 for the Binary cascade, V85-V90 and
V98-V105 for FTFP.
...
| `G4GeneratorPrecompoundInterface::PropagateNuclNucl` reached through `G4TheoFSGenerator` by an ion projectile; `G4FTFModel::AdjustNucleons`; `DecayStrongResonances`; the anti-nucleus projectiles; the lambda branch of `G4HadronNucleonXsc` | what the FTF model still refuses: every ion at 3 GeV per nucleon and above, an anti-nucleon below 1 GeV/c per nucleon, anti-nuclei, lambdas | Phase 3, in progress (P11c). The model runs to a final state for nucleons, pions, kaons and anti-nucleons on any target ([`docs/PORTED.md`](docs/PORTED.md) 2.1.11); nothing calls it from the transport yet |
