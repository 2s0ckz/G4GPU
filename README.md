...
| Geant4 | what | test | worst deviation |
|---|---|---|---|
| `G4Fancy3DNucleus` with `G4NuclearFermiDensity`, `G4NuclearShellModelDensity` and `G4FermiMomentum`; the nuclear fields (`G4ProtonField`, `G4NeutronField`, the pion fields); `G4RKPropagation` with `G4KM_NucleonEqRhs`/`G4KM_OpticalEqRhs`; `G4KineticTrack`; `G4BinaryCascade::ApplyYourself`'s `theBCminP` branch and `G4BinaryLightIonReaction`'s fusion arm | the Binary cascade's foundations, shared with FTF, and the two arms that reach a compound nucleus without a cascade (P9) | `test_bic_nucleus`, `test_bic_apply` | densities 3.2e-16 over 360 points, Fermi momenta 3.8e-15 over 130, the fields 2.4e-14 over 750 with 750 barriers exact, all on replayed nuclei. **`Propagate` and the `im_r_matrix` collision tree are not there** - a nucleon above 45 MeV and every ion above the fusion arm are refused by name (P9b is writing them) |
...
entries V37-V45 for Phase 1, V67-V76 for the Binary cascade's foundations and V85-V90 for the
FTFP half.
...
| `G4BinaryCascade::Propagate` and the `im_r_matrix` collision tree under it (`G4Scatterer`, `G4CollisionManager`, the `G4Collision*` channels, the `G4X*` cross sections, the angular distributions, the resonance widths); `G4BinaryLightIonReaction::Interact` | the Binary cascade proper: a nucleon above 45 MeV (`theBCminP`) and every ion above the fusion arm | Phase 3, in progress (P9b). The foundations under it are on main - the Phase 3 table above and [`docs/PORTED.md`](docs/PORTED.md) 2.1.10 - and the cascade is refused by name at the point it would be needed |
