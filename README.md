...
question; neutron capture and the first wiring of those processes into the steppers are in progress. The cascades and the string model (Phase 3) have not started. The [status
...
driver, test, the viewer and the GUI; runs 68 tests; runs example B1 against a 2M-event Geant4
...
| `G4NeutronRadCapture`, and the wiring of `hadElastic` and the neutron general process | neutron capture; elastic and capture acting in transport | Phase 2, in progress (P7, P8b) |
...
tests/        68 tests; test_custom_hook is a whole project, built as a user would build one
