...
class against the library, and not yet reached by a particle. Phase 2 has landed single Coulomb scattering and closed the negative-hadron question; PreCompound,
neutron capture and the first wiring of those processes into the steppers are in progress. The cascades and the string model (Phase 3) have not started. The [status
...
driver, test, the viewer and the GUI; runs 67 tests; runs example B1 against a 2M-event Geant4
...
| `G4CoulombScattering` + `G4eCoulombScatteringModel` | single Coulomb scattering, the discrete partner of WentzelVI: e± above 100 MeV, every charged hadron from 100 eV | `test_coulomb_scattering` | cross section exact (0 to 5.5e-15) on 17,520 points at both cuts; sampler χ²/bin 2.87 over 4.8 M draws |
...
Everything in this table is transcribed and checked against the library, and nothing in it is
reached by a particle yet: the processes are wired into the steppers in Phase 2. Until that
lands, every number here is a class-level comparison and the B1 and Bragg-peak figures above are
unchanged by it - which is the check that the foundations cost nothing.
...
| `G4Decay`, `G4DecayTable`, `G4PhaseSpaceDecayChannel`, `G4MuonDecayChannel`, `G4KL3DecayChannel`, `G4DalitzDecayChannel`, `G4NeutronBetaDecayChannel` | decay for π±, π⁰, μ±, K± and the neutron | `test_decay` | every deterministic comparison exactly 0; spectra within 3.7 σ over 2,160 bins |
...
| μ±, π±, K±, p̄, deuteron, triton | **transported**, electromagnetically: dE/dx, range, delta rays, radiative losses and fluctuations act; no hadronic process or decay acts on them yet (Phase 2's first wiring) |
| neutron, π⁰ | **transported** by the neutral kernel: the neutron streams to the world boundary under the 10 µs tracking cut with no process acting yet; π⁰ waits for decay to be wired |
...
| `G4PreCompoundModel`, `G4NeutronRadCapture` | pre-equilibrium emission, neutron capture | Phase 2, in progress (P6, P7) |
...
3. **Nothing hadronic acts yet.** Phase 1 is validated class by class; the first like-for-like
   run with hadronic processes acting - QBBC with only the inelastic and at-rest processes
   inactivated, against the port with elastic, capture and decay wired - is Phase 2's P8 and has
   not run. Until it does, the hadronic rows above are transcriptions with oracles, not doses.
...
tests/        67 tests; test_custom_hook is a whole project, built as a user would build one
