...
foundations and its collision tables, and the whole FTFP chain - the tuned parameters, the Glauber
...
entries V37-V45 for Phase 1, V67-V76 and V91-V94 for the Binary cascade, V85-V90 and V98-V105
for FTFP.
...
| `G4BinaryCascade::Propagate` and what is left of the `im_r_matrix` tree under it (`G4Scatterer::Scatter`, the `G4CollisionNNTo*` and `G4Concrete*` resonance channels with `G4XResonance` and `G4Clebsch`, `G4CollisionMesonBaryon`, the resonance widths, `G4KineticTrack::Decay`, `G4Absorber`, the `G4BC*` actions); `G4BinaryLightIonReaction::Interact` | the Binary cascade proper: a nucleon above 45 MeV (`theBCminP`), every pion, and every ion above the fusion arm | Phase 3, in progress (P9c). The foundations and the collision tables under it are on main - the Phase 3 table above and [`docs/PORTED.md`](docs/PORTED.md) 2.1.10 - and the cascade is refused by name at the point it would be needed |
