...
next largest is a nucleon on a HYDROGEN target (`Propagate1H1`). The [status tables](#physics)
below say exactly what is there and what is not.
...
driver, test, the viewer and the GUI; runs 96 tests; runs example B1 against a 2M-event Geant4
...
processes, and V185-V187 for the Gaussian every one of them draws.
...
| Geant4 | what | status |
|---|---|---|
| `G4BinaryCascade::Propagate1H1` | a nucleon or pion on a HYDROGEN target | Phase 3, P9. Reached now that `protonInelastic` and the pion processes are wired, and not a corner in water, where hydrogen is two atoms in three: 107 of the 156 refused interactions over P15's validation grid since P9e (of 1,021 before it) - the largest named hole now, 0 of 93 for a 210 MeV proton in water and 31 of 256 at 1 GeV. `had::HadronicRefusal::kBinaryHydrogenTarget` |
...
tests/        96 tests; test_custom_hook is a whole project, built as a user would build one
