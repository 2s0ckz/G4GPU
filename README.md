...
foundations - the nucleus model, the nuclear fields, the Runge-Kutta propagator and the two arms
that reach a compound nucleus without a cascade - and FTFP's bottom half - the tuned parameters,
the Lund string fragmentation and the string decay - are on main, validated class by class, and
not yet reached by a particle. The cascade proper, the Bertini cascade and the FTF model above its
fragmentation are being written. The [status tables](#physics) below say exactly what is there and
what is not.
...
| `G4FTFParameters`; `G4LundStringFragmentation`, `G4VLongitudinalStringDecay`, `G4HadronBuilder`, `G4FragmentingString`; `G4ExcitedStringDecay`, `G4SampleResonance`, `G4ExcitedString` | FTFP's tuned parameters, the Lund string fragmentation and the string decay (P11) | `test_ftf_params`, `test_ftf_lund`, `test_ftf_strings` | 152,576 exact comparisons, worst 4.4e-16, 4,920 rows refused by name; 70,555 comparisons of which 26,400 are draw counts and 889 statistical; 11,254 over the string-decay layer. **Nothing yet produces a string for it to fragment**: `G4FTFModel`, the participants, the diffractive excitation, the annihilation and `G4TheoFSGenerator` are not written (P11b is writing them) |
...
entries V37-V45 for Phase 1, V67-V76 and V91-V94 for the Binary cascade and V85-V90 for the
FTFP half.
...
| `G4FTFModel`, `G4FTFParticipants`, `G4DiffractiveExcitation`, `G4ElasticHNScattering`, `G4FTFAnnihilation`, `G4TheoFSGenerator` | the FTF model above its fragmentation: from a projectile and a nucleus to the strings, and the generator that runs the chain | Phase 3, in progress (P11b). The parameters, the Lund fragmentation and the string decay are on main and exact (2.1.11); the hand-over `G4GeneratorPrecompoundInterface::Propagate` is ported (2.1.4, `P`: its QGS arm and the anti-nucleus and hypernucleus arms of `PropagateNuclNucl` are refused by name) |
