...
class, and not yet reached by a particle. FTFP runs to a final state for nucleons, pions, kaons,
...
entries V37-V45 for Phase 1, V67-V76, V91-V94, V106-V113 and V148-V157 for the Binary cascade,
V85-V90, V98-V105, V114-V117 and V145-V147 for FTFP, V118-V141 for Bertini and V142-V144 for the
at-rest processes.
...
| `G4BinaryCascade::Propagate` and what is left under it (`G4Scatterer::Scatter`, `G4KineticTrack::Decay` with the width evaluation, `G4Absorber`, `G4MesonAbsorption`, the `G4BC*` actions); `G4BinaryLightIonReaction::Interact` | the Binary cascade proper: a nucleon above 45 MeV (`theBCminP`), every pion, and every ion above the fusion arm | Phase 3, in progress (P9d). The foundations and the whole collision tree under it are on main - the Phase 3 table above and [`docs/PORTED.md`](docs/PORTED.md) 2.1.10 - and the cascade is refused by name at the point it would be needed |
...
| `G4DecayStrongResonances` - `G4KineticTrack::Decay` over the string products, with the decay tables of the 27 short-lived species FTFP makes; the anti-nucleus arm of `PropagateNuclNucl` | what the FTF model still refuses: the decay of its strong resonances, which costs 80% of ion events and 32% of sub-GeV anti-nucleon events, and the anti-nucleus projectiles past the string stage | Phase 3, in progress: the decay engine is P9d's, shared with the Binary cascade, and P11d wires it. The model runs to a final state for nucleons, pions, kaons, anti-nucleons and ions on any target ([`docs/PORTED.md`](docs/PORTED.md) 2.1.11); nothing calls it from the transport yet |
| `G4HadronicAbsorptionFritiof` as reached: the at-rest capture of anti-nucleons, anti-hyperons and anti-nuclei | what `G4StoppingPhysics` still refuses | Phase 3, remaining: P11d builds FTFP's caller-side workspace, then P12's arm runs. The Bertini and muon arms are on main ([`docs/PORTED.md`](docs/PORTED.md) 2.1.13) and nothing calls them from the transport yet |
