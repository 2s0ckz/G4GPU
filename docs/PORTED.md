...
| G4Scatterer (`GetTimeToInteraction`, `Scatter`, `GetCollisions`, `GetCrossSection`, `FindCollision`), G4CollisionManager, G4CollisionInitialState, the six `G4CollisionNNTo*` channels and the `G4Concrete*` tree under them, G4VScatteringCollision, the `G4X*Table` resonance tables, G4ResonanceNames, G4BaryonWidth, G4BaryonPartialWidth, G4PartialWidthTable, G4DetailedBalancePhaseSpaceIntegral | y | **-** | not ported |
...
148,172 comparisons; every cross section and every angle bitwise, the elastic
final states to 2.4e-15 at worst). Device probes, never
