...
| G4ICRU73QOModel | y | **T** | `em/icru73qo.cuh`. In the dispatch, the range tables and now the transport: pi-, K-, pbar and mu- have kernels. The plumbing that was missing is done; what the model's own numbers still carry is the charge-odd air anomaly in `tests/test_hadron_range.cu` (pbar 25% and K- 20% in one band, in `G4_AIR` only). **And an unclosed charge-odd DOSE split, for every negative hadron.** Example B1 at 200 MeV, 500,000 events a side (`ref/b1hadron/`): the port's mu+, pi+ and kaon+ agree with Geant4 EM-only to -0.34%, -0.17% and +0.18% (2.25, 1.16 and 1.21 sigma) and its mu-, pi- and kaon- are +3.78%, +5.01% and +9.52% high (24.7, 32.7 and 61.0 sigma). The port separates its own charge pairs by 0.22-0.27%, which is what Geant4's own restricted dE/dx table implies (0.023% in water, 0.233% in the scored bone); Geant4's transport separates its own by 4.36%, 5.43% and 9.62%. Excluded by measurement: msc (`pion_nomsc.mac`), the at-rest and hadronic processes (all inactivated, and it survives with nothing inactivated), a Bragg peak (all three species cross B1's 300 mm envelope), and the tables (identical step lengths on a fixed seed). It is about 2% in the pure continuous loss (`pion_nodelta.mac`) and gone by 1 GeV (`pion_1gev.mac`). docs/RISK.md V39. Not P1's to fix; the largest open item P1 leaves. |
...
| Geant4 class | QBBC | | Where |
|---|:--:|:--:|---|
| G4MuBetheBlochModel | y | **T** | `em/hadron_ionisation.cuh` (`tests/test_muon.cu`). mu± are transported. The model was exact all along; what was not was the *dispatch* - `hadron_ioni_dedx` did not forward the shell tables to it, so the muon's range table and every transported dE/dx lacked the shell correction and `G4EmCorrections::HighOrderCorrections`. Since the high-order term is charge-odd (Barkas ~ z³), the port gave mu+ and mu- a bit-identical B1 dose where Geant4 separates them by 4.36% (re-measured at 500,000 events; the 4.6% first quoted was a 20,000-event figure). Fixed; mu+ dE/dx 6.397% → 0.861%, mu- 10.176% → 2.209%, and the port now separates its two muons by 0.223%. What it did **not** fix is the reference's 4.36% - see the G4ICRU73QOModel row in 1.1 and docs/RISK.md V39, where the same split is measured for pi- and kaon- and multiple scattering is excluded. docs/RISK.md V38. |
