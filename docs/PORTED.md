...
| **G4BinaryCascade::ApplyYourself** above `theBCminP`, statistically, against `bic_apply.csv` (P9d) | y | **V** | `bic/binary_cascade.cuh`. The 46 MeV rows on C12 for both nucleons - the first two cases in this package where the cascade rather than the compound nucleus answers - now run and agree: 1,145 species yields and 578 species kinetic energies over 5,000 events a case, worst 3.44 and 3.19 sigma, and neither of the two worst is a cascade row. Until this row the 46 MeV cases were refused by name and the test asserted the refusal. **Finding while wiring it:** a cascade product carries a PDG code and the framework wants (Z, A) as well; with those left at zero every proton and neutron the cascade emitted was filed under (Z=0, A=0) and `n46_C12` came out 1,726 neutrons short at 30 sigma |
