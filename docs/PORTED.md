...
| G4IntraNucleiCascader, G4CascadParticle, G4CascadeRecoilMaker, G4CascadeCheckBalance, G4CascadeCoalescence, G4CascadeHistory, G4InuclCollider (`photonuclearOkay`) | yes | **-** | not yet. `bertini/workspace.cuh` defines the buffers and their capacities, and `G4CascadParticle`'s geometry (`getPathToTheNextZone`, the reflection counter) is in `nuclei_model.cuh` |
| G4NonEquilibriumEvaporator, G4EquilibriumEvaporator, G4Fissioner, G4BigBanger, G4CascadeDeexcitation, G4InuclNuclei | yes | **-** | not yet. Reached only by the kaon/hyperon instance - the p, n, pi+ and pi- instances de-excite through P6 |
...
`test_bertini_collide.cu` - 173,061 comparisons, worst 2.5e-14, everything else exact. Device
probes: `bertini_tables_probe` 68 registers / 288 bytes, `bertini_nucleus_probe` 60 / 208,
`bertini_angdst_probe` 56 / 200, `bertini_collide_probe` 158 / 416, no spills, 1,811,828 bytes
of gmem for the tables.
