...
| the workspace **sizing**, and its unit | **V** | The workspace lives for ONE `apply_yourself` call, so a run needs one per thread INSIDE it - not per track in the pool, not per event. `entry::Workspace` (ion beams, `kMaxProjA = 64`) is **433,376 B**; `entry::HadronWorkspace` (`kMaxProjA = 1`, for P12 and P13) is **352,368 B**; `LundTables<double>` is 9,648 B and shared once for the run. 64 slots is 26.46 MB, 256 is 105.81 MB, 1,024 is 423.23 MB, and the per-TRACK reading of a 65,536-track batch would be **27.1 GB** - past every card this project targets, which is why the contract makes the caller state `n_slots` and refuses a thread past the end (`kNoWorkspaceSlot`) instead of aliasing two threads onto one workspace. Both grew by 22,552 B in P11d part 2 for `G4DecayKineticTracks`' own list; a 1 GB budget now buys 2,477 slots |
...
| **G4DecayStrongResonances::Propagate** and **G4DecayKineticTracks** over the string products (P11d part 2) | **V** | `ftf/theo_fs_generator.cuh`'s `ftf_decay_kinetic_tracks`, over P9d's contract header `bic/kinetic_decay.cuh`. It is the FIRST line of all three arms in 11.1.1 - G4GeneratorPrecompoundInterface.cc:148 and :484, and the opening statement of `G4DecayStrongResonances::Propagate` - so it runs once before the arms divide, which is also the only placement that keeps the deviate order: every `G4UniformRand` in that file is at :191 or :538-571, all after the pass. What remains of `G4DecayStrongResonances` after the decay is a copy into `G4ReactionProduct`s with `SetMass(PDGMass)`/`SetTotalEnergy(t())`/`SetMomentum(vect())` - the same three fields the precompound interface fills - so one emitter serves both. **This is what unblocked the package:** the 96-point generality sweep went from 44 ran / 52 refused to **96 ran / 0 refused**, the ion arm from 7,081 of 36,000 (19.7%) to **35,675 (99.10%)**, the sub-GeV anti-nucleon arm from 24,366 (67.7%) to **36,000 (100%)**, and `tests/test_ftf_entry.cu` from 7,563 ran / 12,437 refused to **20,000 ran / 0 refused** - with baryon number and charge exact in every one. Secondaries per ion event went 14 -> 33.03 (docs/RISK.md V160). **Refused by name**, three things the engine does not itself check: `kDecayEngineUnknownSpecies`, a track Geant4 would decay whose code the table lacks - enumerated rather than sampled, it is exactly `-3314` and `-3324`, and they fire in 2 of 36,000 ion events (V159); `kDecayEngineRefused`, the engine's own flags MINUS `unknown_species`, which is benign and would otherwise refuse every eta' (V158) - 323 of 36,000, all of them `list_full` at `kMaxTracks = 256`; and `kDecayLeftShortLived`, the post-pass re-test, for the two ways Geant4 leaves a resonance undecayed without setting anything a caller can read |
