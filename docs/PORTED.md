...
| `G4StoppingPhysics` | `G4HadronStoppingProcess`, `G4HadronicAbsorptionBertini`, `G4HadronicAbsorptionFritiof`, `G4MuonMinusCapture`, `G4EmCaptureCascade` | the process, the element selection, the atomic cascade, muon capture with its bound decay and the Bertini absorption arm **V/P** (2.1.13); `G4HadronicAbsorptionFritiof` **P** - refused until P11d gives FTFP a caller-side workspace; not wired |
