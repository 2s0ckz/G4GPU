...
| **G4BinaryLightIonReaction::ApplyYourself**, the fusion arm | y | **P** | `bic/light_ion_reaction.cuh`, entry point `bic::blir_apply_yourself()`, with `SetLighterAsProjectile`, `FuseNucleiAndPrompound` and `EnergyAndMomentumCorrector`. 20 cases of {d, alpha, C12} on {C, O, Al, Fe, Pb, H} at 1-45 MeV/nucleon x 5,000 events: the fusion gate's verdict exact in all 20 including the one that returns the primary ALIVE (alpha on H at 1 MeV/nucleon - Li5 is unbound), compound (Z, A) exact in 100,000 events, energy balance 2e-10 MeV/event, species yields 3.01 sigma and kinetic energies 3.19. The rotate-to-lab block is the identity and is not carried: V75. **Refused by name:** nothing in this arm. The CASCADE arm is the row below |
| **G4BinaryLightIonReaction::Interact**, `GetProjectileExcitation`, `SortResult`, `DeExciteSpectatorNucleus` and `ApplyYourself`'s cascade arm (P9e) | y | **P** | `bic/light_ion_reaction.cuh`, `bic::blir_cascade_arm` over `bic::propagate`. Complete and compiling: the 150-try loop, both nuclei rebuilt per try on ONE shared `Nucleus3DScratch` (CLHEP's cached Gaussian survives from the first `Init` to the second), the impact parameter in the disc of radius `tOuter + pOuter`, one `outside` track per projectile nucleon at `(0, 0, |p|/pA, E/pA)` with `SetProjectilePotential(-Efermi)`, the split on `IsParticipant()` into spectators and cascaders, the E/p correction loop with its energy test and its momentum give-up, and the spectator fragment through P3's handler with `holes = pA - spectatorA` and zero excitons. The oracle is built and installed - `bic_blir_tape.csv`, `bic_blir_tapeval.csv`, `bic_blir_tapefs.csv` and `bic_blir_init.csv`, 20 events of {d, alpha, C12} on {C12, O16, Al27} at 60 and 200 MeV/nucleon, the whole `ApplyYourself` call recorded uniform by uniform. **NOT YET VERIFIED, and the cause is now found and is not in this file: docs/RISK.md V180.** `G4RandGauss` is `#define`d to `CLHEP::RandGaussQ`, the table-driven quick Gaussian - one uniform per value, no cache - and the port implements `CLHEP::RandGauss`, the polar method - 2.55 uniforms per pair, with a cache. The only caller is the C12 alpha-cluster branch of `ChoosePositions`, so a deuteron or alpha projectile replays BITWISE and the C12 target that follows it does not: 174 uniforms against Geant4's 164, 220 against 186, 122 against 137, positions differing from the first nucleon and the sorted radii by up to 0.6 fermi. A C12 built FIRST, from an untouched stream, differs the same way, so it is the sampler and not the continuation. Nothing in `tests/test_bic_nucleus` could see it - `bic_nucleons.csv` is a REPLAY set and `bic_nucleus_stats.csv` is a distribution. Until `RandGaussQ::transformQuick` is transcribed with its own oracle the ion cascade cannot be compared event by event, so no test asserts it and this row is **P**. The oracle for the comparison is built and waiting: `bic_blir_initnuc.csv` carries both nuclei nucleon by nucleon and a third built from a fresh stream, which is the experiment that located this. **Finding:** the `G4Nucleon*` `G4KineticTrack` constructor fills `theFermi3Momentum` from the nucleon and `Set4Momentum` zeroes it one line later, so a projectile nucleon carries no Fermi motion at all (V179) |
