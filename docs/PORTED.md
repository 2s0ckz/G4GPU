...
| **G4KineticTrack's resonance machinery** - the decay tables themselves, `G4SampleResonance::GetMinimumMass`, `theActualWidth[]` with `G4Integrator::Simpson` and the four integrands, `EvaluateTotalActualWidth`, `SampleResidualLifetime` and the K0 -> K0S/K0L substitution (P9d) | y | **V** | `bic/im_r/decay.cuh` over `im_r/decay_tables.hh`, generated by `tools/extract_bic_decay.pl`. 6,317 table fields, 563 minimum-mass recursions, 15,147 actual widths, 48 substitutions and 2,160 residual lifetimes - **all bitwise** except the lifetimes at 3.6e-16, one ulp from deriving hbar as `hbarc/c_light` the way CLHEP does. The species set is the TRANSITIVE CLOSURE of the cascade's production set under "is a decay daughter of": 94 species, 563 channels, and it has to be, because N(1520) decays to a Delta. The decay tables come from Geant4's own `G4DecayTable` through the oracle rather than from a source array, because `G4ExcitedBaryonConstructor::CreateDecayTable` assembles them at run time - the extractor's header says so and its assertions are structural. **Findings:** ten species' branching ratios do not sum to one and two are cascade resonances (V148); two of the four width branches cannot be reached and both carry defects (V149); building a track from a K0 draws a random and changes the particle (V150). **Refused by name:** four-daughter channels, which the closure does not contain and the extractor asserts against; `G4Integrator`'s adaptive methods, which G4KineticTrack cannot reach |
| **G4KineticTrack::Decay**, **G4GeneralPhaseSpaceDecay** (`DecayIt`, `OneBodyDecayIt`, `TwoBodyDecayIt`, `ThreeBodyDecayIt`, `Pmx`), **G4SampleResonance::SampleMass** and the `G4DynamicParticle` energy-momentum round trip every product goes through (P9d) | y | **V** | `bic/im_r/decay.cuh`. 5,658 product identities and 2,829 draw counts EXACT, 11,316 four-momentum components at 3.2e-16, over 28 resonances x 3 actual masses x 2 parent momenta x 8 prescribed phases - 2,406 two-body products and 423 three-body. The components are judged against the product's own energy, because a decay product's lab momentum crosses zero as the boost cancels the CM momentum and a relative comparison there divides an ulp by a cancellation. **Findings:** the products come back in the REVERSE of the order the phase space made them, which is the order the cascade's track list then takes (V151); two normalisations inside the three-body kinematics divide by a vector that is already unit (V152); and the K0 substitution runs again on every decay product, so a Lambda K0 channel draws one extra uniform after the kinematics (V150). **Refused by name:** `ManyBodyDecayIt`, which needs a four-daughter channel and the closure has none; `G4DecayProducts`' own bookkeeping, which is a container and not physics |
