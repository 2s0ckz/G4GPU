...
| G4UniversalFluctuation | y | **T** | `em/fluctuation.cuh`. **Both `G4RandGauss::shoot` calls - `SampleFluctuations`' thick-absorber Gaussian (.cc:138) and `SampleGauss` (.hh:156) - are `CLHEP::RandGaussQ` through the shared `core/rand_gauss_q.cuh` since P17**, one uniform per trial where the port drew a Box-Muller pair; `tests/test_rand_gauss_q.cu` asserts the count and the value off a tape at both. `g4_gamma`'s internal normal is not a `G4RandGauss` site and keeps its Box-Muller bits; `g4_gamma` itself (Marsaglia-Tsang standing in for `CLHEP::RandGamma`'s GD) is refused by name in docs/RISK.md V185 |
...
| G4IonFluctuations | y | **T** | `em/ion_fluctuation.cuh`, `data/yang_fluctuation.hh` - the alpha's and every ion's fluctuation model. Its `G4RandGauss::shoot` (.cc:149) is `CLHEP::RandGaussQ` through `core/rand_gauss_q.cuh` since P17 - one uniform per trial, asserted in `tests/test_rand_gauss_q.cu` - where it drew the Universal model's Box-Muller pair (docs/RISK.md V185) |
...
| G4HadronNucleonXsc (PDG, NS, all three Kaon* forms, **HyperonNucleonXscNS**) | **P** | `hadronic/xs/hadron_nucleon_xsc.cuh`. The hyperon branch was added by **P11c**, because QBBC gives FTFP a lambda beam from 3 GeV and an anti-lambda beam at every energy (`ftf_windows.csv`) and the model asks this class for its cross sections: without it six points of FTFP's 96-point generality sweep could not run. It is a coefficient per \|PDG\| - 0.88, 0.76, 0.64 for one, two and three strange quarks, and six more for the charm and bottom hyperons - applied to the PROTON's `HadronNucleonXscNS`, scaling total, elastic AND inelastic. `had_hnxsc.csv` gained ten hyperons x two nucleons x every energy already scanned, and `test_ftf_model.cu` compares the `Dispatch` rows. **Still refused:** `SCBMesonNucleonXscNS` (`kSCBMesonNucleonXscNS`), the charm and bottom MESONS, which no QBBC beam reaches |
...
*(P13: NOT any more. `xs/chips_photonuclear.cuh` IS that parameterisation - bit-exact over
10,918 points against `emextra_photonuc.csv` - and `pxs_element_xs` / `pxs_iso_xs`' gamma arms
now evaluate all four branches that used to refuse, including the `coeff[Z][A-amin[Z]]`
continuity factor Z <= 2 gets above 10 GeV. See section 2.1.14. The refusal survives only for a
Z outside 1..98, where this port has no NIST mean atomic mass. The 1,128 and 760 are the count
before that, and this paragraph is left standing because it is what the gap was.)*
...
| G4CompetitiveFission, G4FissionBarrier, G4FissionProbability, G4FissionParameters, G4FissionLevelDensityParameter | **V** | `deexcitation/fission.cuh`; 20,000-odd points, worst 2.1e-14. `EmittedFragment`'s exception after 100 failed splits is refused by name on the output, because a kernel cannot throw. **`FissionCharge` (.cc:292) and `FissionKineticEnergy` (.cc:363) call `G4RandGauss::shoot(mean, sd)`, which is `CLHEP::RandGaussQ`, through `core/rand_gauss_q.cuh` since P17** - one uniform per value, no pair and no state - where the port carried `CLHEP::RandGauss`'s polar method with a `GaussCache` threaded through both; the cache and its parameter are gone, and the count is asserted off a tape in `tests/test_rand_gauss_q.cu` (docs/RISK.md V185) |
...
**THE SHARED CONTRACT CHANGED ON 2026-09-19, AND P11 NEEDS TO KNOW.** Two bugs in the nucleus
model were found by the only kind of test that can see them - a replay of a recorded Geant4
stream - and fixing them moves numbers for every caller, P11's FTF included.

  * **Shape.** `Nucleus3DScratch` lost `gauss_cached` and `gauss_next`. They mirrored
    `CLHEP::RandGauss`'s thread-local statics, and `G4RandGauss` is `CLHEP::RandGaussQ`, which
    has no state at all (V180). A caller that never named those fields compiles unchanged; the
    contract's old advice that a scratch SHOULD be shared across `nucleus_init` calls is now
    simply irrelevant, because nothing is carried either way. `nucleus_model.cuh` says so.
  * **Numbers.** Every C12 built anywhere now consumes Geant4's number of uniforms and puts its
    nucleons where Geant4 puts them (V180) - no other nuclide has a Gaussian in its position
    sampler. And every nucleon of EVERY nuclide now gets a different Fermi momentum VECTOR
    (V181): the same count of draws, the same distribution, the three components reversed,
    because Geant4 builds the trial as one `G4ThreeVector(a, b, c)` and MSVC evaluates a call's
    arguments right to left.

**Is FTFP's oracle affected?** `ref/oracle/` does not move - the oracle is Geant4 and Geant4 did
not change. What moves is the PORT's answer. A P11 test that compares a distribution, a
histogram or a moment sees no difference at all, in either fix. A P11 test that replays a
recorded stream through `nucleus_init`, or that compares a single sampled nucleus nucleon by
nucleon against a dumped one, was failing before and should now pass - and if any such test was
TUNED to the old behaviour it will fail and the tuning is what is wrong. P9's own suite went
from "the ion cascade cannot be replayed at all" to twenty taped events of
`G4BinaryLightIonReaction::ApplyYourself` consuming exactly the recorded number of uniforms.

| Geant4 class | QBBC | | Where |
|---|:--:|:--:|---|
| **G4Fancy3DNucleus** (`Init`, `ChooseNucleons`, `ChoosePositions` incl. the A=12 alpha-cluster branch, `ChooseFermiMomenta`, `ReduceSum`, `CenterNucleons`, `GetNuclearRadius`, `GetOuterRadius`, `GetMass`, `CoulombBarrier`, `StartLoop`/`GetNextNucleon`, both `DoLorentzBoost`/`DoLorentzContraction` overloads, both sorts) | y | **V** | `bic/nucleus/fancy_3d_nucleus.cuh`. Radii, mass, barrier and binding energy exact on 12 nuclides; 5 replayed configurations checked for the hard core, the proton count, the per-nucleon binding energy, the off-shell invariant and the outer radius, all exact; the SAMPLING checked at 20,000 nuclei x 5 nuclides against radial and momentum histograms and five moments, worst 3.20 sigma of 400 bins and 2.15 of 25 moments. C12's cluster spread is a variance used as a sigma: docs/RISK.md V68. `ReduceSum`'s verdict is returned rather than discarded: V74. **Two stream bugs fixed 2026-09-19, neither of which any test in this suite could see** - `G4RandGauss` is `CLHEP::RandGaussQ`, not `CLHEP::RandGauss` (V180), and the three Gaussians of the C12 branch are evaluated RIGHT TO LEFT by the compiler that builds the oracle (V181). Both now have oracles of their own: `bic_gaussq.csv`, 4,027 points over all three branches of `transformQuick`, bitwise; `bic_argorder.csv`, the three uniforms and the vector Geant4 built from them |
...
| **CLHEP::RandGaussQ** (`transformQuick`, `transformSmall`, `shoot`, `shoot(mean, stdDev)`) | y | **T** | **`core/rand_gauss_q.cuh` plus the generated `core/randgaussq_table.hh` since P17 - ONE header shared by every `G4RandGauss` call on the port's path**: the C12 branch of `G4Fancy3DNucleus` below, `G4UniversalFluctuation` (two sites), `G4IonFluctuations`, `G4UrbanMscModel::Randomizetlimit`, `G4WentzelVIModel`'s displacement and `G4CompetitiveFission` (two sites), the EM ones transported, hence `T`. P9e's transcription moved there unchanged and `fancy_3d_nucleus.cuh` re-declares its names; every BIC and FTF test is byte-identical across the move. New oracle `gaussq_transform.csv` (5,735 points, every node of both tables with its neighbours, the engine-lattice tail values and the NaN arguments) and `gaussq_shoot.csv` (32,000 values of both `shoot` overloads on a recorded HepJamesRandom stream, one uniform each), all BITWISE in `tests/test_rand_gauss_q.cu`, which also asserts the draw count at every site; RandGaussQ is measured against the exact normal in docs/RISK.md V185 - variance 1.000135, worst interpolation error 1.6e-2 in z, tail bounded by the engine lattice (6.338 on Philox<double>) and not by the table. (Was `bic/nucleus/fancy_3d_nucleus.cuh` plus `bic/nucleus/randgaussq_table.hh`, 1,250 float entries, extracted by `tools/extract_randgaussq.pl` from CLHEP's `gaussQTables.cdat` with the five geometry constants read out of `RandGaussQ.cc` and asserted.) Not in the hadronic tree at all, and here because `Randomize.hh` line 47 is `#define G4RandGauss CLHEP::RandGaussQ` and `G4Fancy3DNucleus::ChoosePositions` calls it. 4,027 oracle points BITWISE, covering the series tail (`r <= 2e-6`), the fine table, the coarse table, both mirrors about 0.5 and the exact-zero return, with the branch census asserted so a thinned oracle fails instead of passing on the branches it kept. The float truncation at the end of `transformQuick` is part of the answer: removing it fails the comparison at 1.6e-7. docs/RISK.md V180 |
| G4FermiMomentum | y | **V** | `bic/nucleus/fermi_momentum.cuh`; 130 points, 3.8e-15. Not 1e-15, and the reason is `src/data/g4pow.hh`'s A13, not this file: V73. `GetMomentum`'s rejection trial is `uniform_triplet`, a named function, because THE LAST COMPONENT TAKES THE FIRST UNIFORM: Geant4 writes the trial as one `G4ThreeVector(a, b, c)` and MSVC evaluates a call's arguments right to left (V181). Reversing it changes no draw count and no distribution, so `bic_argorder.csv` is the only thing in the suite that can fail when it is wrong |
...
| G4KineticTrack (the two constructors BIC uses, both momentum pairs, `GetActualMass`, the four `Update*Momentum`, `CascadeState`) | y | **P** | `bic/kinetic_track.cuh`. `theFermi3Momentum` is loaded from the nucleon and discarded two lines later, in every event: V69. The resonance-width machinery and the K0 coin flip that this row used to refuse are in `bic/im_r/decay.cuh` (P9d), two rows below |
...
| **G4BinaryLightIonReaction::ApplyYourself**, the fusion arm | y | **V** | `bic/light_ion_reaction.cuh`, entry point `bic::blir_apply_yourself()`, with `SetLighterAsProjectile`, `FuseNucleiAndPrompound` and `EnergyAndMomentumCorrector`. 20 cases of {d, alpha, C12} on {C, O, Al, Fe, Pb, H} at 1-45 MeV/nucleon x 5,000 events: the fusion gate's verdict exact in all 20 including the one that returns the primary ALIVE (alpha on H at 1 MeV/nucleon - Li5 is unbound), compound (Z, A) exact in 100,000 events, energy balance 2e-10 MeV/event, species yields 3.01 sigma and kinetic energies 3.19. The rotate-to-lab block is the identity and is not carried: V75. **Refused by name:** nothing in this arm. Flipped from P to V on 2026-09-19: the P was never about this arm, it was that the model as a whole refused everything above the gate, and the row below now answers that |
| **G4BinaryLightIonReaction::Interact**, `GetProjectileExcitation`, `SortResult`, `DeExciteSpectatorNucleus` and `ApplyYourself`'s cascade arm (P9e) | y | **V** | `bic/light_ion_reaction.cuh`, `bic::blir_cascade_arm` over `bic::propagate`: the 150-try loop, both nuclei rebuilt per try on ONE shared `Nucleus3DScratch`, the impact parameter in the disc of radius `tOuter + pOuter`, one `outside` track per projectile nucleon at `(0, 0, |p|/pA, E/pA)` with `SetProjectilePotential(-Efermi)`, the split on `IsParticipant()` into spectators and cascaders, the E/p correction loop with its energy test and its momentum give-up, and the spectator fragment through P3's handler with `holes = pA - spectatorA` and zero excitons. **TWENTY ION REACTIONS REPRODUCED EXACTLY, FROM THE FIRST NUCLEON OF THE FIRST NUCLEUS TO THE LAST EVAPORATED GAMMA.** The oracle is a TAPE: `ref/dump/dump_bic.cc` wraps CLHEP's HepJamesRandom to record every value it serves across the whole public `ApplyYourself` call, with the REAL precompound model on both of its exits, and the test replays it - so the DRAW COUNT is part of the comparison and a port one uniform out of step fails on its very next number. All twenty events consume exactly the recorded number of uniforms (358 of 358, 1261 of 1261, 2114 of 2114, ... 20380 of 20380), with the status, the secondary count, every secondary's PDG and creator-model id exact, and its four-momentum at 1.4e-11 of its own total energy. Getting there cost two stream bugs that nothing else in this project could have found: docs/RISK.md **V180** (`G4RandGauss` is `CLHEP::RandGaussQ`, not `CLHEP::RandGauss`) and **V181** (the compiler that builds the oracle evaluates a call's three arguments right to left). **THE CAMPAIGN:** {d, alpha, C12} at {50, 200, 1000} MeV/nucleon on {C12, O16, Al27, Fe56}, plus **Fe56 on Al27 and on Fe56 at 200 and 1000 MeV/nucleon** - forty cases, 20,000 events each, 800,000 ion reactions - in `bic_blirapply.csv` and `bic_blirapply_status.csv`. All forty are `stopAndKill` and the verdict is exact; the compound's total charge and baryon number are exact and CONSTANT over every event of every case; species yields worst 4.02 sigma, kinetic energies 3.31, multiplicity 3.50. **The ion arm does NOT conserve energy, and the test says by how much instead of excusing it:** `EnergyAndMomentumCorrector` is a fixed-point iteration with `ErrLimit = 1e-6` and a frozen-iteration escape with no bound at all, so the event is short by `|Scale|` times the energy it started with - asserted PER EVENT, worst ratio 1.000 over 797,584 events, where one means the deficit IS the corrector's residue and nothing else. **Refused by name:** `FillVoidNucleusProducts`, 2,416 events - the destroyed-nucleus branch P9d already refuses, which on a light target at high energy is percents rather than thousandths: 5.87% of C12 on C12 at 1000 MeV/nucleon, every one of them that name. Those events are the most violent end of the distribution, so the three cases that refuse more than 1 in 100 are named, listed with their rate, and LEFT OUT of the three statistical buckets rather than compared through a hole - what is asserted about them is that every refusal carries that one name. **Findings:** the `G4Nucleon*` `G4KineticTrack` constructor fills `theFermi3Momentum` and `Set4Momentum` zeroes it one line later, so a projectile nucleon carries no Fermi motion at all (V179); a cascade residual of ONE nucleon loses its excitation energy outright (V182); the kernel frame is 63,088 bytes against the transport's 16,384-byte limit (V183); and `EnergyAndMomentumCorrector` accepts a relative error of 1e-6 with an unbounded frozen-iteration escape - which is what the per-event bound above is made of - and ABSORBS the rest mass `GenerateGamma` creates for an internal conversion, so an ion event balances with no electron term at all where the nucleon path needs one (V184) |
...
| **The 306 `G4Concrete*` channel instantiations**, the six `G4CollisionNNTo*` composites and the nine per-multiplet children under two of them, **G4CollisionComposite::BufferCrossSection** and **G4CrossSectionBuffer**, and `FinalState`'s selection (P9d) | y | **P** | `bic/im_r/channels.cuh`. 6,624 partial cross sections, 13,248 selections, 1,024 grid points and 768 buffer nodes, **all bitwise**, plus 613 structural checks - the count is exactly 306 and every channel balances charge. The six resonance composites have no cross-section source, so their totals come from a 32-point cache, and two of them buffer a sum of buffers: docs/RISK.md V109. **Refused by name:** the concrete channels' FINAL STATES, which need `G4VScatteringCollision::FinalState` and `SampleResonanceMass` - this file gives the selection, not what comes out of it |
| **G4VScatteringCollision::FinalState** with **SampleResonanceMass** and the `BrWig` inverse, the resonance masses and widths, and **G4VAnnihilationCollision::FinalState** (P9d) | y | **P** | `bic/im_r/resonance_fs.cuh`, masses and widths by `tools/extract_bic_imr.pl`. 29,952 outgoing four-momentum components and 14,976 draw counts and refusal flags, **all bitwise**, over twelve channels drawn from all six families x 39 energies x 8 phases, plus 555 annihilation components and 216 species properties checked against Geant4's own definitions. delta- is 117 MeV wide where its three partners are 120, and thirteen of twenty-four resonance names differ from the mass they are built with - two of them swapped: docs/RISK.md V110. Two branches of `SampleResonanceMass` are unreachable and the test asserts it. **Refused by name:** the `chargeBalance` printout; `BrWigInt1`, called by nothing; `G4XAnnihilationChannel`, so the meson-baryon to-resonance channel still cannot be SELECTED even though its final state is here |
| **G4XAnnihilationChannel** with G4BaryonWidth, G4BaryonPartialWidth and `G4Clebsch::NormalizedClebschGordan` (P9d) | y | **P** | `bic/im_r/xsec_annihilation.cuh`, two 25 x 120 width tables by `tools/extract_bic_imr.pl`. 5,390 mass-dependent widths and 2,695 cross sections, **bitwise**, over all 25 channels `G4CollisionMesonBaryonToResonance` builds x two pion charges x 77 energies. This is the only cross section a pion in the cascade has. Two of the twenty-five fall back to a CONSTANT width because their map key is missing - `N1700_Npi` clobbered by a duplicate `D1700_Npi` assignment, and no `N(2250)` key at all: docs/RISK.md V111, asserted from the oracle's own data. `hbarc` must be DERIVED as CLHEP derives it; the PDG decimal is 1.25e-7 out. **Refused by name:** the strange resonances' columns, which nothing reaches; the particle-antiparticle halving, dead for a meson-baryon pair |
| **G4BinaryCascade::FindFragments**, **DeExcite**, **DecayVoidNucleus**, **ProductsAddFinalState**, **ProductsAddPrecompound** and **G4FermiPhaseSpaceDecay** (P9d) | y | **V** | `bic/cascade_deexcite.cuh`. 3,584 four-momentum components and 896 draw counts from `G4FermiPhaseSpaceDecay::Decay`, **all bitwise**, over 6 body counts from two to eight x 4 parent masses x 8 ladder phases - a DIRECT comparison, because that class is public and default-constructible. BIC is the only class in 11.1.1 that instantiates it (P3's `fermi_breakup.cuh` records that the Fermi break-up model itself does not), and it is reached only for an all-neutron residual nucleus. **Finding:** `BetaKopylov`'s rejection loop has NO iteration guard, and the eight-value prescribed cycle every other sweep in this package uses hangs it - measured, the dump ran 338 rows and stopped dead on the five-body case (V157); that one sweep now uses a 64-value ladder and the port carries a guard Geant4 does not. The exciton count handed to the precompound model is `theCapturedList.size()` and NOT particles-minus-holes; the `excitons-holes` beside it is commented out. `DecayVoidNucleus` invents two MeV per nucleon when the remnant is below threshold, which the source's own comment calls a cheat. **Refused by name:** the `G4HadronicException` in `DeExcite`'s A == 1 arm, which a kernel cannot throw |
| **G4BinaryCascade::StepParticlesOut** and **CorrectFinalPandE** (P9d) | y | **P** | `bic/cascade_finish.cuh`. 17 assertions on `CorrectFinalPandE`, which is where the checkable content is: the scale factor is `max(0.98, pInCM/|pFinals|)`, so it is either exactly 1 or in [0.98, 1), every final's momentum is scaled by exactly it, and every final's INVARIANT MASS is preserved because the new energy is built from the track's own lab `mag2()` and not from its PDG mass. **The 0.98 is a FLOOR, not a tolerance:** an outgoing set carrying more than about 2% too much momentum is corrected by 2% and no more, and the imbalance survives - `Propagate` calls the function up to five times while the excitation energy is negative, and drops the event if that is not enough. `StepParticlesOut` steps by the soonest exit time TIMES 1.2 (a step to exactly the intersection leaves the track on the surface, where `GetSphereIntersectionTimes` reports a miss and the loop would not advance), re-runs `FindCollisions` after 100 barren steps, and after 100 of those resets abandons whatever is left into the final state uncorrected. **Refused by name:** nothing |
| **G4BinaryCascade::FindCollisions**, **FindDecayCollision**, **FindLateParticleCollision** and **ApplyCollision**, with `G4Scatterer::GetCollisions` and `G4CollisionInitialState`'s target collection (P9d) | y | **P** | `bic/cascade_find.cuh`. 22 structural assertions - neither function has an oracle of its own, because every number in them comes from a piece compared elsewhere and `G4BinaryCascade` exposes neither, so what is checked are the facts a plausible port would get silently wrong: `theImR`'s order (decay, absorption, scatterer - and the decay is the only one that draws a uniform); that a RESONANCE with a nucleon target is in charge of NEITHER composite, so the only thing that can happen to it inside the nucleus is its own decay; the target collection's size, 0 for a decay, 1 for a scattering, 2 for an absorption; and the buffer cache reusing a slot per pair of definitions and refusing rather than overwriting when it is full. **The four differ in what the products INHERIT:** an elastic final state makes COPIES of the entrance tracks - keeping position, formation time, state and the `theNucleon` pointer that makes `Hit()` mark the nucleon - where a resonance-production one makes new tracks at the entrance positions with no nucleon. **Refused by name:** nothing new |
| **G4BinaryCascade::DoTimeStep**, **UpdateTracksAndCollisions**, **CorrectBarionsOnBoundary** and `G4KineticTrack::UpdateTrackingMomentum` (P9d) | y | **P** | `bic/cascade_step.cuh`. 160 counter comparisons EXACT and 144 corrections at 3.6e-14, over 4 nuclei x 2 directions x 5 crossing sets. **Finding:** a resonance crossing INTO the nucleus is charged at the proton mass and the same resonance crossing OUT at the neutron mass - 1.293 MeV apart, in a term divided among the simultaneous crossings and added to each one's energy (V156). The two failure branches undo the Coulomb barrier in opposite directions, and a pion that cannot get out is left entirely uncorrected. The cascade's four track lists became ONE POOL with a tag per track, because Geant4's identity is a pointer and `G4CollisionManager` holds one per scheduled collision: a list that shifted on removal would invalidate every collision in it. `DoTimeStep` itself is validated through `Propagate`. **Refused by name:** nothing |
| **G4BinaryCascade::Absorb** and **Capture**, with **G4Absorber::WillBeAbsorbed** and **G4RKPropagation::GetBarrier** (P9d) | y | **P** | `bic/cascade_capture.cuh`. 96 capture verdicts and 1,263 absorption predicates EXACT, 32 captured energies at 9.8e-16, over 4 nuclei x 8 synthetic secondary lists. **Findings:** `Absorb()` cannot absorb anything - `theCutOnPAbsorb` is 0 MeV and the predicate is a KINETIC energy below it, so it is false for every track that has ever existed and the whole of `G4Absorber` is dead code in 11.1.1; and `Capture()` is all-or-nothing, decided by the MEAN energy of every nucleon inside against 0.2*`theCutOnP` = 9 MeV, because the `if(energy < theCutOnP)` that would have separated the slow ones and the `particlesAboveCut==0 &&` that used the result are both commented out upstream. **Refused by name:** `G4Absorber`'s four functions, with the constant that makes them unreachable measured rather than assumed |
| **G4BinaryCascade::CheckPauliPrinciple**, **CorrectShortlivedPrimaryForFermi** and **CorrectShortlivedFinalsForFermi**, with **G4RKPropagation::GetField**'s map default (P9d) | y | **V** | `bic/cascade_collision.cuh`. 1,200 Pauli verdicts EXACT and 3,600 threshold components at 3.8e-15, over 5 nuclei x 12 radii x 2 nucleons x 10 momenta; 366 of the rows block. The function is private, so `write_imr_pauli` re-expresses it from the three public ingredients the source names, in the arithmetic the source writes. The block is against the nucleus AS BUILT and not the current one, the energy uses the product's ACTUAL mass, and the `return false` inside the loop is commented out upstream so every product is examined. The two Fermi corrections branch on the CODE - `abs(pdg) > 1000 && != 2112 && != 2212` - which makes an ANTI-nucleon one of them and a pion not; 8 assertions pin that. **Refused by name:** nothing |
| **G4BinaryCascade's state**: the four track lists, `BuildTargetList`, `BuildLateParticleCollisions`, `GetIonMass`, `GetFinal4Momentum`, `GetFinalNucleusMomentum` and `GetExcitationEnergy` (P9d) | y | **P** | `bic/cascade_state.cuh`. 6,670 ion masses over the whole (Z, A) triangle a cascade can leave behind, **bitwise**, plus 7 branch assertions for the three arms of `GetIonMass` that read no table - a charge above the mass number falls back to A PROTONS, a neutral remnant to A neutron masses - and for `theCutOnP` always coming out at 45 MeV (V72). The rest of the file is state that only `Propagate` can exercise, and it is validated there. **Refused by name:** the `GetPrimaryProjectile()` branch of `BuildLateParticleCollisions`, which is non-null only when a high-energy generator owns this cascade; QBBC's BIC is standalone |
| **G4Scatterer::Scatter** and **G4CollisionComposite::FinalState**'s dispatch through the whole tree, with **G4ConcreteMesonBaryonToResonance::GetOutgoingParticle** and **G4VAnnihilationCollision::FinalState** (P9d) | y | **V** | `bic/im_r/collision_final_state.cuh`. 1,970 product identities and 985 draw counts EXACT, 3,940 four-momentum components at 3.4e-15, over 9 pairs x 9 energies from 50 MeV to 5 GeV x 8 prescribed phases, with a tilted projectile and a moving target so that every rotation and boost in every leaf is live. The selection is THREE deep and each level draws its own uniform against a different kind of number - the top and middle levels read 32-point buffers and the concrete channels are evaluated at the tracks as given - so a port that flattened the tree would match the distribution and diverge on the event. **Finding:** destroying one `G4Scatterer` empties the static channel list for every later one, and `G4BinaryCascade` builds two and deletes both (V155); the port cannot have it, because the table and the buffers are the caller's. **Refused by name:** nothing |
| **G4KineticTrack's resonance machinery** - the decay tables themselves, `G4SampleResonance::GetMinimumMass`, `theActualWidth[]` with `G4Integrator::Simpson` and the four integrands, `EvaluateTotalActualWidth`, `SampleResidualLifetime` and the K0 -> K0S/K0L substitution (P9d) | y | **V** | `bic/im_r/decay.cuh` over `im_r/decay_tables.hh`, generated by `tools/extract_bic_decay.pl`. 7,158 table fields, 638 minimum-mass recursions, 17,118 actual widths, 48 substitutions and 2,160 residual lifetimes - **all bitwise** except the lifetimes at 3.6e-16, one ulp from deriving hbar as `hbarc/c_light` the way CLHEP does. The species set is the TRANSITIVE CLOSURE of the UNION of THREE seed groups under "is a decay daughter of" - the cascade's production set, FTFP's 27 short-lived species (docs/HADRONIC_PLAN.md section 9.3), and every hadron Geant4 marks NOT short-lived: **178 species, 702 channels**, 95 of them short-lived. It has to be a closure, because N(1520) decays to a Delta; the third group is there so that `KineticDecayRefusal::unknown_species` means what it says - without it the engine reported eta', Omega-, the anti-Xis and every charm and bottom hadron as unknown, where Geant4 walks past them in silence (P11d measured that a caller obeying the flag would refuse eta' at 0.011% of FTFP's pion-beam products). The decay tables come from Geant4's own `G4DecayTable` through the oracle rather than from a source array, because `G4ExcitedBaryonConstructor::CreateDecayTable` assembles them at run time - the extractor's header says so and its assertions are structural. **Findings:** ten species' branching ratios do not sum to one and two are cascade resonances (V148); two of the four width branches cannot be reached and both carry defects (V149); building a track from a K0 draws a random and changes the particle (V150). **Refused by name:** `G4Integrator`'s adaptive methods, which G4KineticTrack cannot reach. Four-daughter channels are NOT refused - the closure has four of them (f2(1270) -> 4 pi twice, and the two Lambda_b), the width branch is `BR * PDGWidth` as Geant4 does for anything that is not two or three daughters, and `ManyBodyDecayIt` handles the kinematics in the row below |
| **G4KineticTrack::Decay**, **G4GeneralPhaseSpaceDecay** (`DecayIt`, `OneBodyDecayIt`, `TwoBodyDecayIt`, `ThreeBodyDecayIt`, `Pmx`), **G4SampleResonance::SampleMass** and the `G4DynamicParticle` energy-momentum round trip every product goes through (P9d) | y | **V** | `bic/im_r/decay.cuh`. 10,226 product identities and 5,113 draw counts EXACT, 20,452 four-momentum components at 4.5e-16, over 51 resonances x 3 actual masses x 2 parent momenta x 8 prescribed phases - 4,486 two-body products, 579 three-body and 48 four-body. The components are judged against the product's own energy, because a decay product's lab momentum crosses zero as the boost cancels the CM momentum and a relative comparison there divides an ulp by a cancellation. **Findings:** the products come back in the REVERSE of the order the phase space made them, which is the order the cascade's track list then takes (V151); two normalisations inside the three-body kinematics divide by a vector that is already unit (V152); and the K0 substitution runs again on every decay product, so a Lambda K0 channel draws one extra uniform after the kinematics (V150). `ManyBodyDecayIt` is in too, for f2(1270) -> 4 pi, the only four-daughter channels in the closure - and it reads the daughters' PDG masses where the two- and three-body forms read the sampled ones. **Refused by name:** nothing; `G4DecayProducts`' own bookkeeping is a container and not physics |
| **G4DecayKineticTracks** and the CONTRACT HEADER for the shared decay engine (P9d) | y | **V** | `bic/kinetic_decay.cuh` - the one file a caller outside `bic/` includes, in the shape of `bic/nucleus/nucleus_model.cuh`: plain structs in, plain structs out. 1,134 list shapes and draw counts, 1,134 bookkeeping fields and 1,512 four-momentum components over 7 lists x 8 phases, **all bitwise** but the momenta at 4.1e-16. Built once for two callers on P11c's measurement (docs/HADRONIC_PLAN.md section 9): `G4DecayStrongResonances::Propagate` and `G4GeneratorPrecompoundInterface::Propagate` both open with this pass, and its absence was the ONLY refusal in 52 of FTFP's 96 sweep points, 80% of its ion events and 32% of its sub-GeV anti-nucleon ones. The list-level loop is where the ORDER is decided - it walks a list that grows, appends daughters at the end, and compacts - and that order is the cascade's track list and therefore every later random (V151). **Refused by name:** nothing; a species the table does not carry sets `unknown_species` and is left alone rather than guessed at |
| **G4MesonAbsorption** (`GetCollisions`, `GetTimeToAbsorption`, `AbsorptionCrossSection`, `FindAndFillCluster`, `GetFinalState`), **G4BCDecay** and **G4BCLateParticle** (P9d) | y | **V** | `bic/im_r/absorption.cuh`. 3,416 scheduling verdicts, 582 collision times, 1,749 cluster decisions, 1,728 outgoing components and 648 identities and draw counts, **all bitwise**. The verdicts come from an impact-parameter scan - 8 pairs x 7 energies x 61 offsets in 0.025 fm steps - because `GetTimeToAbsorption` and `AbsorptionCrossSection` are both private and the only door open is the geometry that uses them: `distance <= sigma/pi` flips at one offset, so the scan measures the cross section. **Findings:** the cross-section table is read at `it[-2]` whenever a pi- lands in the second argument, because an `!=` stands where an `==` was meant, and the out-of-bounds values cancel algebraically to exactly `it[1]` - confirmed against Geant4 at 1.96 mb (V153, latent: `FindCollisions` always passes the pion as the projectile); and the absorbing pair's second nucleon is chosen by the smallest `|r1 + r2|`, a sum where a distance would be a difference, so it is the FARTHEST candidate and not the nearest (V154). **Refused by name:** `G4MesonAbsorption::GetCollision`, declared public and never defined or called anywhere in 11.1.1 |
| **G4CollisionMesonBaryonToResonance** (its 25 `G4ConcreteMesonBaryonToResonance` channels and their shared `IsInCharge`), **G4CollisionMesonBaryon**'s buffered total over both components, and `G4CollisionComposite::FinalState`'s selection between them (P9d) | y | **V** | `bic/im_r/collision_meson.cuh`. 3,000 partial cross sections, 3,000 buffered totals and 24,000 selections, **all bitwise**, over 5 pion-nucleon pairs x 300 kinetic energies from 10 MeV to 3 GeV x 8 prescribed phases, plus 160 structural node checks. All 25 channels share one generic-type `IsInCharge`, so the composite sums all 25 for every pair and what separates them is the cross section alone: `NormalizedClebsch` returns 0 where the isospin forbids the state, and that - nothing else - is what stops `GetOutgoingParticle` throwing (docs/RISK.md V110). The tree is two buffers deep and the outer one loses its top node, because the inner buffer returns zero at its own last grid point: docs/RISK.md V112. The sweep runs to 3 GeV and not 1.5 because the elastic partial is identically zero below 1865 MeV (V107) and a shorter one could not tell a buffered elastic from a raw one - MEASURED: it passed 1,500 of 1,500 with the elastic wrongly buffered. A sixth pair, pi+ Delta++, carries the disagreement between the two components' `IsInCharge`: parton count accepts it and generic type does not, so the composite is elastic-only there and its total is ZERO at every energy up to 1120 MeV - docs/RISK.md V113 |
| `G4Scatterer::Scatter`, `GetCollisions`, `GetFinalState`; G4ResonanceNames' name-to-particle map and G4PartialWidthTable's label map, which the port replaces with the column indices `tools/extract_bic_imr.pl` resolves at extraction time | y | **-** | not ported. Everything else this row used to list - the to-resonance composite and its selection, G4XAnnihilationChannel, the six `G4CollisionNNTo*` channels and the 306 `G4Concrete*` under them, G4VScatteringCollision, G4BaryonWidth, G4BaryonPartialWidth - is in the rows above |
| **G4BinaryCascade::Propagate**, **DeExcite** and **ApplyYourself** with both of its retry loops - the whole cascade, end to end (P9d) | y | **V** | `bic/cascade_propagate.cuh` over the fifteen layers below it, and `bic/binary_cascade.cuh` for the two loops around it. **Forty cascades reproduced EXACTLY** - {p, n, pi+, pi-} at 400 and 800 MeV on C12, O16, Al27, Fe56 and Pb208 - 729 structural comparisons with none wrong (impact-parameter tries, uniforms consumed, product count, every product's PDG, its `IsParticipant()` flag and its creator id, and the fragment's A, Z, holes, particles and charged), 612 four-momentum components at 8.7e-14 of the product's own energy, 380 fragment quantities at 5.1e-16. The oracle is a **TAPE**: `ref/dump/dump_bic.cc` calls the PUBLIC `G4BinaryCascade::Propagate` with a nucleus it built, under CLHEP's HepJamesRandom wrapped to record every value it serves, and the test replays it - so the DRAW COUNT is part of the comparison and a port one uniform out of step fails on its very next number. Measured: shifting the tape by one changes the number of impact-parameter tries from 2 to 8, the fragment from A=10 to A=11 and a product energy by a factor of 7. The precompound exit is a parameter, called behind `__noinline__`, and the dump replaces it with a model that records the fragment and returns nothing, so what is compared is the cascade and only the cascade. **Findings:** `IsParticipant()` returns true for a track with no nucleon (V163); `GetSpherePoint` starts the projectile 1.5 radii back where its own comment says 1 (V162). **Refused by name:** `FillVoidNucleusProducts`, the destroyed-nucleus branch, 180 lines of ad-hoc corrections with a `G4UniformRand()` in them; `Propagate1H1`, the A == 1 arm, which is a separate 230-line single-nucleon reaction and is what leaves water incomplete; `HighEnergyModelFSProducts` and the late-particle arm, reachable only from a high-energy generator |

**THE REGISTER AND STACK COST**, `nvcc -arch=sm_86 -Xptxas -v` on `tests/test_bic_apply.cu`,
which instantiates both entry points as `__global__` probes that are never launched - for V52's
reason, that a `__host__ __device__` template only ever called from the host is never compiled
...
`TransportEngine::Upload` and `b1_gpu_sched.cu` both set `cudaLimitStackSize` to 16,384. **63,088
is not 16,384**, and a kernel that calls either entry point under that limit overflows its stack
and surfaces as an illegal memory access from an unrelated API call. That is a WIRING
constraint, not a cascade bug, and docs/RISK.md **V183** sets out the three ways out and what
each costs. For scale, `run_step_hadron` is 4,592 B and `run_step_neutral<kNeutron>` is 7,264 B.
| **G4BinaryCascade::ApplyYourself** above `theBCminP` - THE CAMPAIGN, against `bic_apply.csv` (P9d) | y | **V** | `bic/binary_cascade.cuh`. Sixty cascade cases at **20,000 events each** - {p, n} at 100, 400, 800 and 1400 MeV and {pi+, pi-} at 200 and 800 MeV, on C12, O16, Al27, Fe56 and Pb208 - plus the eighteen sub-threshold cases this file started with, 1.96 million events in all. Sixty cascade cases at **20,000 events each** - {p, n} at 100, 400, 800 and 1400 MeV and {pi+, pi-} at 200 and 800 MeV, on C12, O16, Al27, Fe56 and Pb208 - plus the eighteen sub-threshold cases this file started with, 1.96 million events in all. **12,930 species yields, worst 3.892 sigma; 4,197 species kinetic energies, worst 3.759; 97 multiplicities, worst 3.662** - against a 5-sigma bound and an expected maximum near 3.9 for 12,930 draws. The summed (Z, A) of the products and the status verdict are EXACT in all 98 cases. Balance: on the compound path 1.6e-6 MeV against the oracle and 2.0e-10 MeV per event against the port's own compound, the ulp; on the cascade path 0.160 MeV and 3.6e-3 MeV per event, and the second is the one that matters - the port's own per-event balance is 9.1e-12 MeV on a 12 GeV sum, so the 0.160 is Geant4's own mean sitting that far from (beam + target mass) on the four carbon cases at 800 MeV, 1.3e-5 of the energy in the event. 524 events of 1.96 million are refused, 0.027%, and the by-name tally says which: 109 are `FillVoidNucleusProducts`, the destroyed-nucleus branch, and the rest are P6's own. **Water** is in the brief and is not a row: a material is not a target, the framework samples an ELEMENT per interaction, and water offers hydrogen and oxygen - O16 is here and H1 is `Propagate1H1`, refused by name, so water is answered for its oxygen and refused for its hydrogen. **Findings while wiring it:** a cascade product carries a PDG code and the framework wants (Z, A) as well, and with those left at zero every proton and neutron the cascade emitted was filed under (Z=0, A=0) - `n46_C12` came out 1,726 neutrons short at 30 sigma; and the test's own species key split on `a > 0`, which put every Lambda in the neutron bucket while the oracle's Lambda rows went to a key of their own, reading "port 0, g4 128" at 11 sigma on five pion cases for a species the port had right |
...
391,331 comparisons; every cross section, every angle and every interaction time
...
| **G4FTFModel**, G4FTFParticipants, G4DiffractiveExcitation, G4ElasticHNScattering, G4DiffractiveSplitableHadron, G4FTFAnnihilation, G4VPartonStringModel, G4InteractionContent, **G4TheoFSGenerator** | **P** | written by P11b - see 2.1.11b below. `AdjustNucleons` came in with P11c and the strong-resonance decay with P11d part 2, so the only thing left refused in this group is the anti-nucleus hand-over, and that is P6's `GeneratorRefusal::anti_nucleus` |
...
| **G4FTFModel** (`Init`, `GetStrings`, `StoreInvolvedNucleon`, `ReggeonCascade`, `PutOnMassShell`, `ComputeNucleusProperties`, `GenerateDeltaIsobar`, `SamplingNucleonKinematics`, `CheckKinematics`, `FinalizeKinematics`, `ExciteParticipants`, `BuildStrings`, `GetResiduals`, `GaussianPt`) | **P** | `ftf/ftf_model.cuh`. No exact oracle - every path starts from a sampled nucleus - so it is checked statistically, below. `ExciteParticipants`' inelastic-rejection factor is an INTEGER division and reads as a smooth suppression: docs/RISK.md V99. **Both arms of `GetResiduals` are written**, and they are different algorithms: the high one boosts to the residual c.m.s. and shares over the INVOLVED nucleons, the low one shares over the ones that actually COLLIDED and un-hits the rest (docs/RISK.md V115). An **ANTI-NUCLEUS** projectile builds its strings too (P11c): the re-typing Geant4 does after `Init` changes only the PDG code the splitable carries, so a flag and a sign replace it and P9's `bic::Nucleon` needs no anti types (docs/RISK.md V117). **Nothing in this class is refused by name any more**; what stops an anti-nucleus is P6's own `GeneratorRefusal::anti_nucleus` at the hand-over. The annihilation branch of `ExciteParticipants` is written, `theAdditionalString` with it, and the participant pool carries 64 slots for it past the two nuclei |
...
| **G4TheoFSGenerator::ApplyYourself** | **P** | same file, entry point `ftf::apply_yourself()`. The two dummy branches below 100 MeV (a charm/bottom hadron, a hypernucleus) are reproduced rather than refused, because returning the primary IS what Geant4 does. **All three hand-overs are wired**: `Propagate` for a hadron beam, `PropagateNuclNucl` for an ion one since P11c - with `MakeCoalescence` in front of it, both residuals emitted, and the projectile's boosted back to the lab from the rest frame `propagate_nucl_nucl_residuals` leaves it in - and, since P11d part 2, `G4DecayStrongResonances` for the event that hit EVERY target nucleon, which on hydrogen is the only path there is. **Refused by name:** `G4QuasiElasticChannel` and `G4CRCoalescence`, both unreachable in QBBC |
...
| the workspace **sizing**, and its unit | **V** | The workspace lives for ONE `apply_yourself` call, so a run needs one per thread INSIDE it - not per track in the pool, not per event. `entry::Workspace` (ion beams, `kMaxProjA = 64`) is **539,096 B**; `entry::HadronWorkspace` (`kMaxProjA = 1`, for P12 and P13) is **458,088 B**; `LundTables<double>` is 9,648 B and shared once for the run. Both shrank by 8 B when P9e's V180 removed the Gaussian cache from `bic::Nucleus3DScratch`, which the workspace embeds once, and V167's `static_assert`s refused the build until these numbers moved with the types - their first catch. 64 slots is 32.91 MB, 256 is 131.63 MB, 1,024 is 526.48 MB, and the per-TRACK reading of a 65,536-track batch would be **33.7 GB** - past every card this project targets, which is why the contract makes the caller state `n_slots` and refuses a thread past the end (`kNoWorkspaceSlot`) instead of aliasing two threads onto one workspace. A 1 GB budget buys **1,991** `Workspace` slots or **2,343** `HadronWorkspace` ones. BOTH GREW TWICE IN P11d PART 2, and both times for the decay pass: 22,552 B for `G4DecayKineticTracks`' own list (`bic::DecayTrack` is 88 B and is not a prefix of `preco::CascadeTrack`), then 105,728 B more when `kMaxTracks` went 256 -> 512, which bounds five arrays and `StringsWorkspace`'s `kMaxOut` at once. The second rise bought the heavy-ion tail: `list_full` went from 323 of 36,000 ion events to 66, and those 66 are all at ONE of the 72 points, Fe56 on Pb207 at 20 GeV per nucleon (13.2% of its 500 events) - the corner of the grid. Every other beam, target and energy is clean |
| the sizing table as a **compile-time** fact (P11f) | **V** | `entry::kWorkspaceBytes`, `kHadronWorkspaceBytes` and `kLundTableBytes` are `constexpr sizeof`, with `static_assert`s pinning all three and a fourth asserting that `HadronWorkspace` is the smaller type. The table used to be prose and it drifted 24% - it said `HadronWorkspace` was 266,344 B where `sizeof` gave 329,816, so a caller sizing 1,024 slots budgeted 272.8 MB and needed 337.7 MB (docs/RISK.md V167, found by P12b). A change to any array in `FtfWorkspace` now fails the build with a message naming the three places the number lives. Measured that it bites: one byte off the asserted value fails the compile and nothing else does |
| an **ion over the workspace's capacity**, refused by name in ONE attempt (P11f) | **V** | `entry::HadronWorkspace` has `kMaxProjA = 1`, so every ion is over it, and the header has always promised "refused by capacity, with `refused_a` naming the mass number". It did not happen: `ftf_scatter`'s retry guard tested two of `FtfModelReport::any()`'s eight terms and `involved_capacity` was not one, so an unchangeable refusal span all 1,000 attempts and came back `kPrimaryUnchanged` with no name. P12b's timing is the proof - 0.50 ms a call, **flat in Z**, because the capacity test returns before both `nucleus_init` calls, so 1,002 attempts built nothing. The guard is `report.any()` now, which is safe because `ftf_model_init` (ftf_model.cuh:1483-1619) sets only `refused`, `involved_capacity` and `nucleus_failed` - every retryable flag belongs to `ftf_get_strings`, whose guard is deliberately left alone. `entry::Report` gained `capacity_a` because the mass number had nowhere to go. Measured over deuteron, triton, He3 and alpha at 10 GeV on carbon, 50 events each: **0 -> 50 of 50 refused by capacity with the right mass number, 50 -> 0 primary-unchanged, 1002 -> 1 attempt**, with a proton through the same workspace running 200 of 200. docs/RISK.md V168 |
| **`apply` REPLACES the final state, it does not append** | **V** | Stated in the contract header because P12b found it the hard way: `apply_yourself` opens with `out.clear()`, which wiped the EM cascade's gammas `G4HadronicAbsorptionFritiof` had already appended. That is what `G4HadronicInteraction::ApplyYourself` is entitled to assume - Geant4 constructs a fresh `G4HadFinalState` per model call - and a reused workspace makes a stale list the likelier bug, so the behaviour stands and the contract says so. A caller with secondaries of its own keeps them in a local list and re-appends, which is what the Bertini arm already does |
| **G4DecayStrongResonances::Propagate** and **G4DecayKineticTracks** over the string products (P11d part 2) | **V** | `ftf/theo_fs_generator.cuh`'s `ftf_decay_kinetic_tracks`, over P9d's contract header `bic/kinetic_decay.cuh`. It is the FIRST line of all three arms in 11.1.1 - G4GeneratorPrecompoundInterface.cc:148 and :484, and the opening statement of `G4DecayStrongResonances::Propagate` - so it runs once before the arms divide, which is also the only placement that keeps the deviate order: every `G4UniformRand` in that file is at :191 or :538-571, all after the pass. What remains of `G4DecayStrongResonances` after the decay is a copy into `G4ReactionProduct`s with `SetMass(PDGMass)`/`SetTotalEnergy(t())`/`SetMomentum(vect())` - the same three fields the precompound interface fills - so one emitter serves both. **This is what unblocked the package:** the 96-point generality sweep went from 44 ran / 52 refused to **96 ran / 0 refused**, the ion arm from 7,081 of 36,000 (19.7%) to **35,932 (99.81%)**, the sub-GeV anti-nucleon arm from 24,366 (67.7%) to **36,000 (100%)**, and `tests/test_ftf_entry.cu` from 7,563 ran / 12,437 refused to **20,000 ran / 0 refused** - with baryon number and charge exact in every one. Secondaries per ion event went 14 -> 35.93 (docs/RISK.md V160). **Refused by name**, three things the engine does not itself check: `kDecayEngineUnknownSpecies`, a track Geant4 would decay whose code the table lacks - enumerated rather than sampled, it is exactly `-3314` and `-3324`, and they fire in 2 of 36,000 ion events (V159); `kDecayEngineRefused`, the engine's own flags MINUS `unknown_species`, which is benign and would otherwise refuse every eta' (V158) - 66 of 36,000 after `kMaxTracks` was raised to 512, every one of them `list_full` and every one at Fe56 on Pb207 at 20 GeV per nucleon; and `kDecayLeftShortLived`, the post-pass re-test, for the two ways Geant4 leaves a resonance undecayed without setting anything a caller can read. The ion arm runs **35,932 of 36,000 (99.81%)** |
...
An **ANTI-NUCLEUS** - the anti-deuteron, anti-triton, anti-He3 and anti-alpha QBBC's FTFP list
constructs - builds its nucleus, its participants and its strings (300 of 300 events on carbon
and on lead at 1 and 5 GeV/nucleon, 4.7 to 26.0 strings each) and stops at P6's
`GeneratorRefusal::anti_nucleus` at the hand-over, which the P11c brief leaves refused.
**Below 1 GeV/c it runs too**, since P11c: `AdjustNucleons` and the low-energy arm of
`GetResiduals` are written, and over {pbar, nbar} x {H, C, O, Al, Fe, Pb} x {1, 10, 50, 100,
200, 400} MeV, 500 events each, 24,366 of 36,000 produce a final state with baryon number and
charge exactly right and none refused at `AdjustNucleons`. An ION beam builds its projectile nucleus,
...
to a final state, 52 refused BY NAME, none was silent** (it was 11 and 85 before P11c). **All 52
are the same refusal**: `preco::GeneratorRefusal::short_lived_track`, P6's `G4DecayKineticTracks`,
and V100 says why every FTFP event reaches it. `kPropagateNuclNucl`, `kAdjustNucleons` and
`kHadronNucleonXscRefused` have all left the sweep - the ion arm, the sub-GeV arm and the hyperon
cross section are wired - so one gap in one other package is now the whole of what stops FTFP.
...
whole of `ftf::apply_yourself`, is **255 registers, a 1,376-byte stack frame and 8/24 bytes
spilled** in the entry function; `ftf_scatter` compiled on its own spills 48 bytes of stores and
104 of loads. 400 bytes cmem[0], 6,008 bytes cmem[2]. The frame grew from 864 bytes in P11d
part 2 and the reason is the decay engine: `ftf_decay_kinetic_tracks` is behind a `__noinline__`,
but `bic::decay_kinetic_tracks` carries a four-slot daughter buffer and `kinetic_decay_one` a
13-slot width array, and those are frames of their own. The
...
`sizeof(FtfWorkspace<250,64,512,320,512,96>)` is **487,392 bytes** - two nuclei, a 565-slot
splitable-hadron pool (the 64 extra slots are G4FTFAnnihilation's `theAdditionalString`), the
interaction list, 320 excited strings and P11's 74,296-byte string-decay workspace - one per
...
`usePreCompoundDeexcitation()` on both, so those four species de-excite through P6's `preco::`
module. `G4HadronicBuilder::BuildFTFP_BERT` builds a third for K+, K-, K0S, K0L and the six
hyperons over 0-6 GeV and does not, so those run the cascade's own evaporators.
`bertini/cascade_params.cuh` carries all three windows and both de-excitation choices, read from
`ref/oracle/bertini_params.csv` rather than from `G4CascadeParameters`' defaults. docs/RISK.md
V118. The three windows as `qbbc_bertini_range` answers them, each verified against the dump and
pinned in `tests/test_bertini_apply.cu`:

*(P13: it is SIX times, in two configurations. `G4EmExtraPhysics` builds three more - one in
`ConstructGammaElectroNuclear` for the photon over 199 - 6000 MeV, and one inside each of
`G4ElectroVDNuclearModel` and `G4MuonVDNuclearModel` for the equivalent photon below 10 GeV -
and all three are plain `new G4CascadeInterface`, so all three keep `useCascadeDeexcitation()`.
The same residual therefore de-excites through `bertini/deexcite.cuh` when a photon made it and
through P6 when a proton did. `qbbc_bertini_range` is not extended, because the photon's window
is P13's configuration and not this package's: `emextra::qbbc_config(22)` and
`emextra::qbbc_gamma_deexcite_choice()` carry it. docs/RISK.md V170.)*

| species | window | de-excitation | applicable to the model? |
|---|---|---|---|
| p, n | 1,000 - 6,000 MeV | PreCompound (P6) | yes |
| pi+, pi- | 1,000 - 12,000 MeV | PreCompound (P6) | yes |
| K+, K- | 0 - 6,000 MeV | the cascade's own | yes |
| K0S, K0L | 0 - 6,000 MeV | the cascade's own | **NO** - docs/RISK.md V139 |
| lambda, sigma+/0/-, xi0/-, omega- | 0 - 6,000 MeV | the cascade's own | yes |
| everything else, incl. anti-nucleons, light anti-nuclei, photons, muons | - | - | no window at all |

Two rows there are not what the builder alone would say. **K0S and K0L have a window and no model
behind it**: `G4InuclElementaryParticle::type` names `G4KaonZero` and `G4AntiKaonZero` and not the
weak eigenstates, so `IsApplicable` refuses both and every K0S or K0L handed to Bertini is
`NoInteraction` - V139. And **a muon has no window but IS applicable**, because
`G4CascadeMuMinusPChannel` exists; what keeps it out is that QBBC registers no Bertini process on
one. "The physics list offers it" and "the model accepts it" are written in different packages and
nothing in Geant4 compares them, so both are evaluated together in the test rather than read.

**Why the exact half is exact.** Geant4 lets a random engine be installed, so
`ref/dump/dump_bertini.cc` drives the samplers under an eight-value uniform cycle and dumps the
number of deviates each call consumed beside its answer. One sampler cannot be driven that way -
`BetaKopylov` is an unbounded rejection loop taking two deviates per trial, so a period-8 cycle
gives it four distinct trials and none is ever accepted - and the phase-space pass therefore uses
a 64-bit LCG written out in integer arithmetic on both sides. Both engines are reproduced exactly
in `tests/test_bertini_collide.cu`.

| Geant4 class | QBBC | | Where |
|---|:--:|:--:|---|
| **G4CascadeParameters** (+ the three instances QBBC builds, and their energy windows) | yes | **V** | `bertini/cascade_params.cuh`; all 20 parameters and three windows at 17 digits, exact. `doCoalescence` defaults TRUE and four of the radius/scale parameters come from `G4HadronicDeveloperParameters` and not from the initialiser - see docs/RISK.md V118 |
| G4InuclElementaryParticle, G4InuclParticleNames, G4InuclSpecialFunctions | yes | **V** | `bertini/inucl_particle.cuh`; mass, charge, strangeness, baryon number and nine predicates for 43 type codes, plus `inucl_store_momentum` - G4InuclParticle keeps a direction, a kinetic energy and a mass, NOT a four-vector, and the round trip through it is what makes a photon-nucleon collision agree at 2.5e-14 instead of 2e-7 (docs/RISK.md V125) - plus `randomInuclPowers`, `randomCOS_SIN`, `randomPHI`, `G4cbrt` (BOTH overloads - the integer one is `G4Pow::Z13`, the double one is exp/log, and `generateModel` calls both), `nucleiLevelDensity`, `paraMaker`. `G4Pow::powN(0, n)` is 0 for every n including 0, so the INUCL power series collapse at zero kinetic energy - docs/RISK.md V120 |
| **G4CascadeChannelTables** and the 34 `G4Cascade*Channel` data classes, G4CascadeFunctions, G4CascadeSampler, G4CascadeData, G4CascadeInterpolator, G4PionNucSampler / G4KaonSampler / G4KaonHypSampler | yes | **V** | `bertini/channel_tables.cuh` on `src/data/bertini_channels.hh`, generated by `tools/extract_bertini_channels.pl` (counts asserted against the template arguments): 185,970 cross sections, 42,890 final-state codes, 9,472 derived values. 858,362 comparisons, worst 0, **bitwise**. `findFinalStateIndex` returns an absolute index on one path and a relative one on the other, and `G4CascadeMuMinusPChannel` reads off the end because of it - refused by name, docs/RISK.md V119 |
| G4TwoBodyAngularDist and its 15 distributions, G4MultiBodyMomentumDist and its 4, G4InuclParamAngDst, G4InuclParamMomDst, G4VThreeBodyAngDst | yes | **V** | `bertini/angular_dist.cuh` on `src/data/bertini_angdst.hh` (2,541 values); every `ChooseDist` verdict and every `GetCosTheta`/`GetMomentum` under the cycle with its draw count, worst 0 |
| **G4NucleiModel** - `initializeCascad` (both), `choosePointAlongTraj`, `generateNucleon`, `generateInteractionPartners`, `generateParticleFate` | yes | **V** | `bertini/cascade_model.cuh`, on P10's zone tables. 565,916 comparisons - 1,056 entry points and 18,432 cascade steps against `bertini_initcascad.csv` and `bertini_fate.csv` - with draw counts, multiplicities, product types, positions, zones, paths, reflection counters, generations and the nucleon census all **exact** and four-momenta worst 8.4e-15. The grid runs a momentum of ZERO and a radially incident photon because each reaches an expression nothing else does (docs/RISK.md V127). `resetReflection` clears the flag and not the counter - V126 - and no one-step grid can see it, so it is asserted directly. The NUCLEUS-projectile `initializeCascad` is transcribed (A < 5, the only case it handles) but QBBC never gives Bertini an ion, so it has no oracle row and is marked `P` below |
| **G4NucleiModel** - `generateModel` and everything deterministic under it | yes | **V** | `bertini/nuclei_model.cuh`; 6,687 comparisons over 13 nuclei - zone radii and volumes, both adaptive density integrals, nucleon densities, Fermi momenta and kinetic energies, the five-slot potential fold over all 44 type codes, binding-energy differences, `getZone`, `useQuasiDeuteron`'s whole truth table, `totalCrossSection` and `absorptionCrossSection` - worst 0 with **tolerance 0** in all 45 buckets. `passTrailing` cannot reject anything with the dumped `radiusTrailing` of 0, and both halves of that are pinned |
| G4LorentzConvertor | yes | **V** | `bertini/lorentz_convertor.cuh`; no oracle of its own - it is measured through the collider, whose 24,716 final-state four-momenta go through `toTheCenterOfMass`, `rotate`, `rotate(v1,v2)` and `backToTheLab`. `scm_momentum` means the reversed target momentum after one call and the bullet momentum after the other, and `rotate` uses it as the reference axis either way, so the two are separate functions here |
| **G4ElementaryParticleCollider** (`collide`, `generateMultiplicity`, `generateOutgoingPartTypes`, `generateSCMfinalState`, `fillOutgoingMasses`, `generateSCMpionAbsorption`, `generateSCMpionNAbsorption`, `pionNucleonAbsorption`) | yes | **P** | `bertini/ep_collider.cuh`, entry `bert::ep_collide()`. 10,752 cases - 28 particle pairs x 12 lab momenta x 2 target states x 8 phases x 2 generator settings: draw counts, multiplicities, product types and refusals **exact**, four-momenta worst 2.5e-14. The target is run at rest AND with a Fermi momentum, because a head-on collision makes `G4LorentzConvertor::rotate` the identity and the first grid never applied it - docs/RISK.md V124. **Refused by name:** `generateSCMmuonAbsorption` (P12's muon capture), and `generateSCMpionNAbsorption` from this entry point because it needs a residual-nucleus mass the collider is given by `setNucleusState` - the routine itself is transcribed and takes that mass as an argument. Both refusals are exercised, the second by turning `piNAbsorption` on |
...
| **G4CascadeInterface** (`ApplyYourself`, `IsApplicable`, `createBullet`, `createTarget`, `copyOutputToHadronicResult`, `makeDynamicParticle`, `checkFinalResult`, `retryInelasticProton`/`Nucleus`, `coulombBarrierViolation`, `NoInteraction`), **G4InuclCollider** (`collide`, `deexcite`, both de-excitation choices), **G4PreCompoundDeexcitation** | yes | **P** | `bertini/cascade_interface.cuh`, entry point `bert::apply_yourself(HadProjectile, HadNucleus, HadFinalState, ...)` with P5's shapes. Compared STATISTICALLY - three nested retry loops mean no prescribed engine survives to the top - against `bertini_apply.csv` and `bertini_apply_species.csv`: **22,531 comparisons over 95 cases and 190,000 events, every asserted bucket inside its band**, worst 3.2 sigma on the multiplicity, 4.5 on one low-count fragment yield, 4.9 on one fragment spectrum and 3.9 on one kaon angle. Energy and z-momentum are compared relatively (they are conservation identities, not random variables - docs/RISK.md V133) and agree to 6e-7; the oracle's momentum-non-conservation column is `mean_pz` minus a constant and is asserted as that identity, which holds to 3.3e-14 and checks the mass table on the way. Both de-excitation arms are wired AND both are in the grid: p, n, pi+, pi- to P6's `preco::deexcite`, K+, K-, lambda to the cascade's own evaporators, selected by `qbbc_bertini_range(pdg)` - docs/RISK.md V118. The kaon and hyperon cases were added after the first grid, which had nucleons and pions only and therefore never ran `bertini/deexcite.cuh` above the level of its own oracle - docs/RISK.md V124, and they are what measured V135 and then V137. **Refused by name:** `G4LightTargetCollider` (a photon on A < 3 - P13's; *P13: WRITTEN, in `emextra/light_target.cuh`, as a SIBLING of this entry point rather than inside it, because `G4CascadeInterface::ApplyYourself` branches to it before `createBullet` - see 2.1.14. This enumerator stays because it is still the only thing that can report a projectile reaching that branch, and nothing under `bertini/` changed*), hyper-nuclear targets, `throwNonConservationFailure` (a kernel cannot throw: `ApplyResult::would_throw` carries it out and the campaign compares the count against the oracle's `thrown` column, both zero over 190,000 events), and every capacity. `Propagate`/`rescatter` is P11's entry and is not ported. **The refusal rate over the whole grid is 11 events in 190,000** - 0.006%, all of them `kFate` from the nuclei model, none above 0.2% in any case. It was 8,095 until P10c decayed the trapped hyperons instead of refusing them (docs/RISK.md V135, V136), and that is the difference between sixteen cases that could only be reported and sixteen that are asserted and pass |
| **G4IntraNucleiCascader::decayTrappedParticle**, and the seven hyperon `G4DecayTable`s it needs | yes | **V** | `bertini/intra_cascader.cuh` on P4's `decay::sample_decay`, through the lambda / sigma+ / sigma0 / sigma- / xi0 / xi- / omega- rows P10c added to `physics/decay/decay_tables.hh` (section 3). Those rows are compared against `ref/oracle/decay_tables.csv` like every other row there - 30 channels, **worst relative deviation 0.000e+00** on parent mass, width, lifetime, branching ratio, two-body Pmx and daughter mass and width - and `ref/dump/dump_decay.cc`'s `kSpecies` grew from eight entries to fifteen so the new species get the same six comparisons the old eight get. All eleven channels are `G4PhaseSpaceDecayChannel` with two daughters, so no sampler came with them. **Measured over the 190,000-event grid: 10,234 trapped decays** - lambda 7,999, sigma0 2,215, sigma+ 11, xi0 4, xi- 4, sigma- 1, omega- 0. The sigma0 is the only hyperon channel that emits a photon, so a trapped one puts a gamma back on the cascade stack, and lambda -> sigma0 -> lambda is a cycle the cascade runs until the nucleus empties; omega- is never trapped in this grid and its table stands on Geant4's authority rather than on measured need. **Refused by name:** the boost above 1,115.68 GeV, where Geant4's GeV-into-a-MeV-parameter stops being a no-op (`kTrappedHyperonBoost`, docs/RISK.md V136), and a daughter with no `G4InuclParticleNames` code, which Geant4 emits as a null secondary (`kTrappedDaughterNotInucl`); both unreachable for these eleven channels and both evaluated rather than assumed |
| photonuclear and muon-capture entry points, hyper-nuclei | - | **-** | refused by name. `G4CascadeInterface` serves P13 and P12 through the same class, but their callers are not this package's. A **mu- is applicable** - `G4CascadeMuMinusPChannel` exists, so `IsApplicable` says yes - and what keeps it out of a QBBC run is that no Bertini process is registered on a muon at all; `tests/test_bertini_apply.cu` pins both halves, after the first version of the pin asserted the opposite and was corrected against the source rather than the other way round |

Tests: `test_bertini_data.cu` - 865,049 exact comparisons, worst 0 in all 47 buckets;
`test_bertini_collide.cu` - 173,074 comparisons, worst 2.5e-14, everything else exact;
...
worst 2.2e-13; `test_bertini_apply.cu` - 22,531 statistical comparisons over 95 cases and 190,000
events, every asserted bucket inside its band, and a species-coverage bucket that walks the PORT's
species rather than the oracle's so that a species Geant4 never made would be visible (16,711
...
| **G4HadronStoppingProcess** (`AtRestGetPhysicalInteractionLength`, `AtRestDoIt`, `IsApplicable`), **G4HadronicAbsorptionBertini**, **G4HadronicAbsorptionFritiof**, **G4MuonMinusCapture** | yes | **P** | `stopping/stopping_process.cuh`, entry `stopping::at_rest`. The at-rest length is ZERO and NotForced, which is why capture pre-empts `G4Decay` every time and why P4's `decay_at_rest_competitor` says what it says - the two now agree and both are pinned. Reported as a distribution: **16 species x 5 materials x 20,000 events** - every species QBBC gives an at-rest process, including all four anti-nuclei. **The Fritiof arm RUNS** (P12b), through P11d's `ftf/ftf_entry.cuh` and nothing else of the model: the caller holds an `entry::Handle<entry::HadronWorkspace>`, a thread takes slot `tid`, and this single-threaded campaign uses **one slot of 329,816 bytes** - measured with `sizeof`, against the 266,344 the entry header's own table states, docs/RISK.md V165 - or 339,464 from `bytes_for(1)` with the Lund table that slots share. Of **1,000,000** Fritiof calls - 10 species x 5 materials x 20,000 - **480,177 (48.02%) produce a real final state**, 5.52 to 5.77 secondaries from the nuclear model depending on species; **400,000 (40.00%)** come back as `kFtfPrimaryUnchanged`, which is EVERY anti-nucleus event and is the undersized-workspace door V166 mislabels; and **119,823 (11.98%)** are refused with a name: `G4DecayStrongResonances` **37,269** (P11d's, landing in parallel on phys/ftf3) and P6's `GeneratorPrecompoundInterface` **82,554**. Against the anti-BARYONS alone - the six species this port can actually finish - 480,177 of 600,000 run, **80.03%**, and the 19.97% that do not split 6.21% strong-resonance decay and 13.76% hand-over. **Refused by name:** `kReentryExhausted` (AtRestDoIt's 100-attempt loop, which Geant4 answers with a FatalException a kernel cannot throw), `kTrappedDaughterNotInucl`, and `kSecondaryOverflow`. Probe `stopping_at_rest_probe`: 255 registers, 13,456-byte stack frame, 284 bytes of spill stores - down from 34,000 when a `HadFinalState` was still a local, docs/RISK.md V144 |
...
**What the campaign is NOT.** Its multiplicity, deposit and capture-fraction columns are printed
and read, not checked: there is no `stopping_campaign.csv`, because `AtRestDoIt` needs a G4Track in
a G4Material inside a run and the piecewise dumps cannot reach it. So this package differs from the
Bertini one, where `test_bertini_apply` holds its assembly to a dumped oracle over 95 cases inside
five-sigma bands - and an earlier version of `dump_stopping.cc` claimed the two were arranged the
same way. The one assertion that does run through the assembly is `AtomicCascadeSurvives`: a count
that cannot legally decrease. It exists because a plausible wrong number did get through and was
caught by hand off a printed row (docs/RISK.md V164). Closing the gap needs a fourth dump driving
both absorption processes at rest over the same grid with a real engine; it is named in
`dump_stopping.cc` rather than attempted.

Tests: `test_stopping.cu` - **1,089,047 comparisons, 0 failures**, plus the 80-row campaign table.
1,080,164 of those comparisons are `AtomicCascadeSurvives`, one per completed event. The eight
tests this package owns or touches were all rebuilt and re-run for the final report:
...
`test_wiring` PASSED, `test_stopping` 1,089,047 / 0.

**Anti-vacuity, `AtomicCascadeSurvives`.** Run once with its fix removed - the Fritiof arm handed
`fs` straight to `entry::apply` again, which is the bug as it was: the assertion fires on the first
Fritiof species, `anti-p on H2O ... got 6 want 14`, and the run exits non-zero. Nothing else moves:
1,259,476 comparisons, one failing bucket, and every other bucket's worst value identical to the
clean run. The campaign rows show the same impossibility that found the bug by hand - `anti-p C`
reading mean multiplicity 5.750 against mean EM-cascade 10.037, a total smaller than one of its
parts. Fix restored and re-run before commit.
...
#### 2.1.14 G4EmExtraPhysics: the photon, electron, positron and muon on a nucleus (P13)
...
list: every process on gamma, e-, e+, mu- and mu+, from the process manager AND from
`G4HadronicProcessStore` (the gamma's `photonNuclear` is inside `G4GammaGeneralProcess` and is
absent from the manager), every model's `[GetMinEnergy, GetMaxEnergy]`, every data set's name,
and - the column that matters - the high-energy generator inside each `G4TheoFSGenerator`.

| particle | process | cross section | models, in registration order |
|---|---|---|---|
| gamma | `photonNuclear` (inside `GammaGeneralProc`) | `GammaNuclearXS` | `GammaNPreco` 0 - 200, `BertiniCascade` **199** - 6000, `TheoFSGenerator`/**QGS** 3000 - 100000000 |
| e- | `electronNuclear` | `ElectroNuclearXS` | `G4ElectroVDNuclearModel` 0 - 1e9 |
| e+ | `positronNuclear` | `ElectroNuclearXS` | the same OBJECT |
| mu-, mu+ | `muonNuclear` (one process for both) | `KokoulinMuonNuclearXS` | `G4MuonVDNuclearModel` 0 - 1e9 |

Off, and each one asserted by the ABSENCE of its process rather than by reading a flag:
synchrotron (`synActivated`, `synActivatedForAll`), gamma->mumu (`gmumuActivated`), mu->mumu
(`mmumuActivated`), e+->mumu and e+->tautau (`pmumuActivated`), e+->hadrons (`phadActivated`),
all four neutrino processes (`fNuActivated`), and LEND (`gLENDActivated`, which is false and
also unreachable because `ConstructLENDGammaNuclear` is only called in the branch taken when
the gamma general process does NOT exist, and `G4LENDDATA` is unset).

Four corrections to docs/HADRONIC_PLAN.md section 2 and to this document's own 2.2 row:

1. the class is in `physics_lists/constructors/gamma_lepto_nuclear/`, not `electromagnetic/`;
2. **the photon's high-energy generator is QGS**, `G4QGSModel<G4GammaParticipants>` with
   `G4QGSMFragmentation` - not FTFP. docs/RISK.md V169;
3. `GammaNPreco` ends at 200 MeV and Bertini starts at **199**, a deliberate one-MeV seam
   (`cascade->SetMinEnergy(fGNLowEnergyLimit - CLHEP::MeV)`), so a photon there is assigned at
   random with P(Bertini) = (E - 199)/1;
4. QBBC builds Bertini **six** times, not three, and the three this package owns de-excite with
   the CASCADE's own evaporators and not with P6. docs/RISK.md V170.

| Geant4 class | QBBC | | Where |
|---|:--:|:--:|---|
| **G4EmExtraPhysics** (the four processes, their data sets, their models and every window) | y | **V** | `hadronic/emextra/config.cuh`, against `emextra_config.csv`, `emextra_windows.csv`, `emextra_xs.csv` and `emextra_params.csv`: 37 configuration rows, 7 model windows and 4 data-set names exact, 10 off-by-default processes asserted absent, the two transition energies checked against `G4HadronicParameters`, and the model choice counted over 200,000 draws at each of 7 points in the two overlaps against `overlap_probability_upper` - worst 0.00129 |
| **G4PhotoNuclearCrossSection** (GetElementCrossSection, GetIsoCrossSection, ThresholdEnergy, EquLinearFit, GetFunctions) | y | **V** | `hadronic/xs/chips_photonuclear.cuh` on `src/data/chips_photonuclear.hh` (8,281 values by `tools/extract_emextra_chips.pl`, every array's length asserted against the `nL`/`nH`/`nLA`/`nHA` the same source declares). 10,388 element + 530 isotope points, **worst 0 - bit-exact**. The per-Z cache is not ported: `GetFunctions` mixes two table rows with ONE coefficient shared by all 105 (or 224) entries, so the two entries `EquLinearFit` reads are built where they are needed rather than materialising a row to read two of it, and the 0.000e+00 is what says that is identical and not merely equivalent. `GetFunctions`' `else y[q] = 0.` arm is DEAD - hydrogen is the only element with A <= 1.5 and its 144.6821 MeV threshold is above the GDR table's 106 MeV ceiling - and the perturbation that was meant to catch it is what proved it |
| **G4ElectroNuclearCrossSection** (GetElementCrossSection, ThresholdEnergy, GetFunctions, GetEquivalentPhotonEnergy, SolveTheEquation, GetEquivalentPhotonQ2, GetVirtualFactor, HighEnergyJ1/2/3, Fun, DFun) | y | **V** | `hadronic/xs/chips_electronuclear.cuh` on `src/data/chips_electronuclear.hh` (14,112 values). 10,388 element points **bit-exact**, and the three equivalent-photon functions plus their three draw counts over 288 (Z, E, phase) cases under the eight-value cycle, also **bit-exact**. The class is STATEFUL - `GetEquivalentPhotonEnergy()` takes no arguments and reads five members `GetElementCrossSection` left behind - so the state is an explicit `ElnState` and a sampler called on a default one returns zero exactly as Geant4's `lastSig <= 0` guards do. It rounds A to an integer where the photo-nuclear class does not, uses its own seven-digit electron mass and pi, and its `dM` is 3e-6 from the one `G4ElectroVDNuclearModel` uses one line later; all four transcribed as written - docs/RISK.md V172 |
| **G4KokoulinMuonNuclearXS** (ComputeDDMicroscopicCrossSection, ComputeMicroscopicCrossSection, BuildCrossSectionTable, GetElementCrossSection) | y | **V** | `hadronic/xs/kokoulin_muon_xs.cuh`. 540 double-differential points (270 with each of the two arguments its two callers pass - docs/RISK.md V173) and 11,132 element points over 92 elements x 121 energies, worst 7.2e-16. The element cross section is a `G4PhysicsLogVector`, 61 nodes from 1 GeV to 1 PeV, and the transport reads the TABLE: the node rows check the eight-point Gauss-Legendre integration exactly (`ComputeMicroscopicCrossSection` is private and reachable only through it) and the rows between nodes check the interpolation on top |
| **G4GammaNuclearXS** above the data files | y | **V** | `hadronic/xs/particlexs.cuh`, P2's gamma arm finished. The four branches 2.1.1 counted as 1,128 element and 760 isotope refusals are now evaluated: the element straight line to `xs150[Z]`, the element CHIPS value above 150 MeV, the isotope straight line (anchored on CHIPS's ISOTOPE value at 150 MeV, not `xs150[Z]`), and the `coeff[Z][A-amin[Z]]` factor Z <= 2 gets above 10 GeV. Checked against `emextra_gammanuc.csv`: 94 `xs150` values, 92 elements and every isotope the run's elements carry, over a grid that includes 130, 150 and 50000 MeV exactly. `XsRefusal::kPhotoNuclearCrossSection` survives only for Z outside 1..98 |
| **G4LowEGammaNuclearModel** (ApplyYourself) | y | **V** | `hadronic/emextra/low_e_gamma.cuh`, into P6's `preco::deexcite`. The photon is absorbed WHOLE into `G4Fragment(A, Z, p_target + p_gamma)` - the target's own (A, Z), not A+1 - with ZERO excitons, so pre-equilibrium emits nothing and a photo-nuclear reaction below 200 MeV in QBBC is pure evaporation; `n_preco_products == 0` is asserted rather than described. Every secondary is emitted at +1 ns because the model reads `GetTOF()`, which is a sign flag nothing ever sets - docs/RISK.md V171 |
| **G4LightTargetCollider** (collide, GammaDCrossSection, AbsorptionOnDeuteron, SingleNucleonScattering) | y | **V** | `hadronic/emextra/light_target.cuh` - the arm 2.1.12 lists among P10's refusals, written as a SIBLING of `bert::apply_yourself` because `G4CascadeInterface::ApplyYourself` branches to it before `createBullet`; nothing under `bertini/` changed. It matters because two of the three atoms in water are hydrogen and `G4GammaNuclearXS` sends every hydrogen target to CHIPS at every energy. All four reachable arms are asserted with their thresholds bracketed, and all three deuteron channels fire (229/245/26 over 500 draws at 400 MeV). `SingleNucleonScattering` is NOT `G4ElementaryParticleCollider::collide`: the multiplicity is drawn once and walked DOWN, `itry_max` is 200 not 10, and there is no SCM conversion and no sort |
| **G4CascadeInterface** on a photon | y | **V** | `hadronic/emextra/photon_nuclear.cuh` through P10's `bert::apply_yourself`, with `DeexciteChoice::kCascade` - `qbbc_gamma_deexcite_choice()`. `photonuclearOkay` is behind a raw `std::getenv("G4CASCADE_CHECK_PHOTONUCLEAR")` which is unset on this install (`emextra_params.csv` records it, and the test fails if it is ever set), so the filter never runs and is not ported |
| **G4ElectroVDNuclearModel** (ApplyYourself, CalculateEMVertex, CalculateHadronicVertex) | y | **P** | `hadronic/emextra/lepton_vd.cuh`. The equivalent-photon sampling is exact through `emextra_eqphoton.csv`; the three gates, the scattered lepton and the hand-over to the gamma chain are compared statistically. `P` for the >= 10 GeV pi0-into-FTF arm, which is refused by name at the hand-over and is unreachable below a 10 GeV lepton. When it is wired it goes through `ftf/ftf_entry.cuh` and nothing else - P11d's caller-side contract, one 329,816-byte `entry::HadronWorkspace` slot per thread that can have a lepton above 10 GeV in flight, which is not one per lepton and not one overall |
| **G4MuonVDNuclearModel** (ApplyYourself, CalculateEMVertex, CalculateHadronicVertex, MakeSamplingTable) | y | **P** | same file. The 5 x 73 x 800 sampling table is built from `kokoulin::dd_microscopic_xs` term by term, with the `g/mole` factor `MakeSamplingTable` applies and which cancels (docs/RISK.md V173). The `epmax <= CutFixed` return is bracketed at `T = CutFixed + 0.5*m_p - m_mu = 563.478 MeV`, so a 200 MeV muon-nuclear interaction produces NOTHING. `P` for the same FTF arm |
| **G4QGSModel<G4GammaParticipants>** + `G4QGSMFragmentation` + `G4ExcitedStringDecay` | y | **-** | refused by name, `EmExtraRefusal::kQgsGammaString`, over 3 GeV - 100 TeV. P11 ported FTF, which is a different string model. It costs (E - 3000)/3000 of every photo-nuclear interaction in the overlap and all of it above 6 GeV - two thirds at 5 GeV, measured. docs/RISK.md V169 |
| `G4SynchrotronRadiation`, `G4GammaConversionToMuons`, `G4AnnihiToMuPair` (x2), `G4eeToHadrons`, `G4MuonToMuonPairProduction`, the eighteen neutrino classes, `G4LENDorBERTModel` + `G4LENDCombinedCrossSection` | - | **-** | refused by name with the flag value, and each one's ABSENCE from the run asserted |

**What is refused, by name**: the QGS generator above 3 GeV; the two lepton models' FTF arm
above a 10 GeV equivalent photon without a 332 kB workspace; a hyper-nuclear target; the
light-target collider's pion arms (which exist in Geant4 and have no caller in it); a Z outside
1..98 in either CHIPS class; every buffer capacity.
...
| `G4HadronInelasticQBBC` | `G4HadronInelasticProcess`; `G4ParticleInelasticXS`, `G4BGGPionInelasticXS`, `G4NeutronInelasticXS`; models `G4BinaryCascade`, `G4CascadeInterface` (Bertini), `G4TheoFSGenerator` + `G4FTFModel` + `G4ExcitedStringDecay` + `G4QGSModel`, `G4PreCompoundModel`, `G4GeneratorPrecompoundInterface`, `G4ExcitationHandler` | cross sections **V** (2.1.1); `G4ExcitationHandler` **V** (2.1.3); `G4PreCompoundModel` **V**, `G4GeneratorPrecompoundInterface` **P** (2.1.4); the FTFP chain **P** (2.1.11, 2.1.11b: the strong-resonance decay is written since P11d part 2; the anti-nucleus hand-over is refused on P6's side); `G4BinaryCascade` **P** (2.1.10: `Propagate` runs for nucleons and pions; `G4BinaryLightIonReaction::Interact` and the hydrogen target are refused); `G4CascadeInterface` (Bertini) **P** (2.1.12); `G4HadronInelasticProcess` itself and its wiring into the steppers: **none** |
| `G4IonPhysicsXS` | `G4ParticleInelasticXS`, `G4BinaryLightIonReaction` | cross section **V** (2.1.1); `G4BinaryLightIonReaction` **P** (2.1.10: the fusion arm, `Interact` refused); FTFP for ions **P** (2.1.11b); not wired |
| `G4IonElasticPhysics` | `G4ComponentGGNuclNuclXsc`, `G4NuclNuclDiffuseElastic` | **none** |
| `G4StoppingPhysics` | `G4HadronStoppingProcess`, `G4HadronicAbsorptionBertini`, `G4HadronicAbsorptionFritiof`, `G4MuonMinusCapture`, `G4EmCaptureCascade` | the process, the element selection, the atomic cascade, muon capture with its bound decay and the Bertini absorption arm **V/P** (2.1.13); `G4HadronicAbsorptionFritiof` **P** - now RUNS through P11d's `ftf_entry.cuh` (P12b): over 1,000,000 at-rest calls, 48.02% produce a real final state, 40.00% are the four anti-nuclei that this workspace cannot take and that come back unnamed (V166), and 11.98% are refused by name - `G4DecayStrongResonances` and P6's `GeneratorPrecompoundInterface`. Not wired into a stepper |
...
| `G4EmExtraPhysics` | gamma-nuclear (`G4GammaNuclearXS`, `G4LowEGammaNuclearModel`, LEND), electro- and muon-nuclear (`G4ElectroVDNuclearModel`, `G4MuonVDNuclearModel`), 18 neutrino classes, `G4SynchrotronRadiation`, `G4AnnihiToMuPair`, `G4GammaConversionToMuons`, `G4eeToHadrons`, `G4MuonToMuonPairProduction` | **V/P** (2.1.14, P13). All four processes, all three cross sections and all four final-state models, plus `G4PhotoNuclearCrossSection` and `G4LightTargetCollider` which this row did not name and which the chain cannot do without. The two lepton models are `P` for their >= 10 GeV pi0-into-FTF arm. The high-energy generator this row does not name is `G4QGSModel<G4GammaParticipants>`, NOT FTFP, and it is refused by name over 3 GeV - 100 TeV (docs/RISK.md V169). The neutrinos, the synchrotron, the three mu-pair/hadron channels and LEND are OFF in QBBC and each one's absence from the run is asserted rather than assumed. Not wired into a stepper: P15's, and 2.1.14 says what it needs to know |
