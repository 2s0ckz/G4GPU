...
three energies inside each, on carbon and on lead - 96 (beam, energy, target) points. **40 ran
to a final state, 56 refused BY NAME, none was silent** (it was 11 and 85 before P11c). The 56
break down as 50 `preco::GeneratorRefusal` (P6's `G4DecayKineticTracks`, and V100 says why every
FTFP event reaches it) and 6 `kHadronNucleonXscRefused` (the lambda beam, which has no
cross-section branch below this package). **Neither `kPropagateNuclNucl` nor `kAdjustNucleons`
appears in the sweep any more**: the ion arm and the sub-GeV arm are both wired.
